...
IFS=$'\n' apps_ass=($(find "$BUILD_PATH/app" -maxdepth 1 -name "*.app"))
IFS=$'\n' apps_cli=($(find "$BUILD_PATH/cli" -maxdepth 1 -name "*.app"))
IFS=$'\n' apps_dt=($(find "$BUILD_PATH/dt" -maxdepth 1 -name "*.app"))
...
PEP_MACOS_ASS_APP_NAME="$(basename "${apps_ass[0]%.app}")"
PEP_MACOS_CLI_APP_NAME="$(basename "${apps_cli[0]%.app}")"
PEP_MACOS_DT_APP_NAME="$(basename "${apps_dt[0]%.app}")"

APP_DIR="$BUILD_PATH/app/$PEP_MACOS_ASS_APP_NAME.app/Contents/Resources"
CLI_DIR="$BUILD_PATH/cli/$PEP_MACOS_CLI_APP_NAME.app/Contents/Resources"
DT_DIR="$BUILD_PATH/dt/$PEP_MACOS_DT_APP_NAME.app/Contents/Resources"
...
copy_config_files "$CLI_DIR"
copy_config_files "$DT_DIR"
