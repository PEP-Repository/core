...
if [[ ${#apps_ass[@]} -ne 1 ]] || [[ ${#apps_cli[@]} -ne 1 ]]; then
...
copy_config_files() {
    ...
    "$PEP_MACOS_ROOT_DIR/scripts/createConfigVersionJson.sh" "$INFRA_DIR" "$INFRA_DIR/project" "$BUILD_NUM" > "$target_dir/configVersion.json"
