...
notarize_macos_apps() {
  ...
  xcrun notarytool store-credentials -v "notarytool-password" \
  ...
  for PRODUCT_PATH in "${PRODUCT_PATHS[@]}"; do
    ...
    for i in {1..10}; do
      ...
      set +e
      (
        xcrun notarytool submit -v "$PRODUCT_PATH.zip" \
                                        --keychain-profile "notarytool-password" \
                                        --output-format plist > "notarization_submit.plist" && echo 1 > "$SUCCESS_FILE"
      )
