...
# Assumed to be ran in a folder with the arm64 and x86_64 subdirectories, each containing their corresponding macOS_artifacts, with app and cli subdirectories, each containing their corresponding .app bundles
...
if [[ ${#app_files_x86[@]} -ne 1 ]] || [[ ${#app_files_arm[@]} -ne 1 ]] || [[ ${#cli_files_x86[@]} -ne 1 ]] || [[ ${#cli_files_arm[@]} -ne 1 ]]; then
...
if [[ "$(basename "${app_files_x86[0]}")" != "$(basename "${app_files_arm[0]}")" ]] || [[ "$(basename "${cli_files_x86[0]}")" != "$(basename "${cli_files_arm[0]}")" ]]; then
...
mkdir -p "universal/macOS_artifacts/"{app,cli}
...
# Call the function with the two app directories, for PEPAssessor
