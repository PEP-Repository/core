...
# Assumed to be ran in a folder with the app, cli and dt subdirectories, each containing their corresponding .app bundles
...
sign_app() {
  ...
  local codesign_options=(--verbose --force --options=runtime --timestamp --keychain pep.keychain --sign)
  ...
  codesign --verbose --keychain pep.keychain --deep --timestamp --sign "$GITLAB_CI_MACOS_CERTIFICATE_APP_NAME" "$app_path" || true
...
PEP_MACOS_ASS_APP_ROOT="app"
PEP_MACOS_CLI_APP_ROOT="cli"
PEP_MACOS_DT_APP_ROOT="dt"

if ! PEP_MACOS_ASS_APP_PLIST_PATH=$(readlink -f "$PEP_MACOS_ASS_APP_ROOT"/*.app/Contents/Info.plist); then
  echo "Error: Could not find Info.plist for root dir: $PEP_MACOS_ASS_APP_ROOT"
fi
...
if ! PEP_MACOS_DT_APP_PLIST_PATH=$(readlink -f "$PEP_MACOS_DT_APP_ROOT"/*.app/Contents/Info.plist); then
  echo "Error: Could not find Info.plist for root dir: $PEP_MACOS_DT_APP_ROOT"
fi

PEP_MACOS_ASS_APP_NAME=$(/usr/libexec/PlistBuddy -c "Print CFBundleDisplayName" "$PEP_MACOS_ASS_APP_PLIST_PATH")
PEP_MACOS_ASS_APP_IDENTIFIER=$(/usr/libexec/PlistBuddy -c "Print CFBundleIdentifier" "$PEP_MACOS_ASS_APP_PLIST_PATH")
PEP_MACOS_ASS_APP_INSTALL_LOCATION="/Applications"
...
PEP_MACOS_DT_APP_NAME=$(/usr/libexec/PlistBuddy -c "Print CFBundleDisplayName" "$PEP_MACOS_DT_APP_PLIST_PATH")
PEP_MACOS_DT_APP_IDENTIFIER=$(/usr/libexec/PlistBuddy -c "Print CFBundleIdentifier" "$PEP_MACOS_DT_APP_PLIST_PATH")
PEP_MACOS_DT_APP_INSTALL_LOCATION="/Applications"

PEP_MACOS_ASS_APP_INFRA_NAME="$PEP_MACOS_ASS_APP_NAME"
PEP_MACOS_CLI_APP_INFRA_NAME="$PEP_MACOS_CLI_APP_NAME"
PEP_MACOS_DT_APP_INFRA_NAME="$PEP_MACOS_DT_APP_NAME"

# Update name to infra names for non-universal apps (universal is already at infra name by now)
if [[ "$MACOS_INSTALLER_ARCH" != "Universal" ]] && [[ "$PEP_MACOS_ASS_APP_NAME" != *"$INFRA_NAME"* ]] && [[ "$PEP_MACOS_ASS_APP_NAME" != *"$BRANCH_NAME"* ]]; then
  PEP_MACOS_ASS_APP_INFRA_NAME="$PEP_MACOS_ASS_APP_NAME ($INFRA_NAME $BRANCH_NAME)"
  set_or_add_plist "$PEP_MACOS_ASS_APP_PLIST_PATH" "CFBundleDisplayName" "string" "$PEP_MACOS_ASS_APP_INFRA_NAME"
  mv "$PEP_MACOS_ASS_APP_ROOT/$PEP_MACOS_ASS_APP_NAME.app" "$PEP_MACOS_ASS_APP_ROOT/$PEP_MACOS_ASS_APP_INFRA_NAME.app"
  PEP_MACOS_ASS_APP_PLIST_PATH="$PEP_MACOS_ASS_APP_ROOT/$PEP_MACOS_ASS_APP_INFRA_NAME.app/Contents/Info.plist"
  ...
  PEP_MACOS_DT_APP_INFRA_NAME="$PEP_MACOS_DT_APP_NAME ($INFRA_NAME $BRANCH_NAME)"
  set_or_add_plist "$PEP_MACOS_DT_APP_PLIST_PATH" "CFBundleDisplayName" "string" "$PEP_MACOS_DT_APP_INFRA_NAME"
  mv "$PEP_MACOS_DT_APP_ROOT/$PEP_MACOS_DT_APP_NAME.app" "$PEP_MACOS_DT_APP_ROOT/$PEP_MACOS_DT_APP_INFRA_NAME.app"
  PEP_MACOS_DT_APP_PLIST_PATH="$PEP_MACOS_DT_APP_ROOT/$PEP_MACOS_DT_APP_INFRA_NAME.app/Contents/Info.plist"
fi

# Update versions
CONFIG_VERSION_MAJOR="$($PEP_CORE_DIR/scripts/parse-version.sh get-major "$CONFIG_ROOT_PATH/version.json")"
CONFIG_VERSION_MINOR="$($PEP_CORE_DIR/scripts/parse-version.sh get-minor "$CONFIG_ROOT_PATH/version.json")"

PEP_MACOS_APP_CONFIG_VERSION="$CONFIG_VERSION_MAJOR.$CONFIG_VERSION_MINOR.${CI_PIPELINE_ID:-$PEP_MACOS_DEFAULT_VERSION}"
PEP_MACOS_APP_CONFIG_VERSION_LONG="$PEP_MACOS_APP_CONFIG_VERSION.${CI_JOB_ID:-$PEP_MACOS_DEFAULT_VERSION}"

set_or_add_plist "$PEP_MACOS_ASS_APP_PLIST_PATH" "CFBundleVersion" "string" "$PEP_MACOS_APP_CONFIG_VERSION_LONG"
set_or_add_plist "$PEP_MACOS_ASS_APP_PLIST_PATH" "CFBundleShortVersionString" "string" "$PEP_MACOS_APP_CONFIG_VERSION"
...
set_or_add_plist "$PEP_MACOS_DT_APP_PLIST_PATH" "CFBundleVersion" "string" "$PEP_MACOS_APP_CONFIG_VERSION_LONG"
set_or_add_plist "$PEP_MACOS_DT_APP_PLIST_PATH" "CFBundleShortVersionString" "string" "$PEP_MACOS_APP_CONFIG_VERSION"

# Update appcast urls
PEP_MACOS_ASS_APPCAST_URL="$PEP_MACOS_UPDATE_PATH_PREFIX/$PEP_MACOS_ASS_APP_ROOT/AppCast.xml"
set_or_add_plist "$PEP_MACOS_ASS_APP_PLIST_PATH" "SUFeedURL" "string" "$PEP_MACOS_ASS_APPCAST_URL"
...
PEP_MACOS_DT_APPCAST_URL="$PEP_MACOS_UPDATE_PATH_PREFIX/$PEP_MACOS_DT_APP_ROOT/AppCast.xml"
set_or_add_plist "$PEP_MACOS_DT_APP_PLIST_PATH" "SUFeedURL" "string" "$PEP_MACOS_DT_APPCAST_URL"

# Create PEP Assessor app
create_app "$PEP_MACOS_ASS_APP_ROOT" "$PEP_MACOS_ASS_APP_INFRA_NAME" "$PEP_MACOS_ASS_APP_INSTALL_LOCATION" "$PEP_MACOS_APP_CONFIG_VERSION_LONG" "$PEP_MACOS_ASS_APP_IDENTIFIER"
...
create_app "$PEP_MACOS_DT_APP_ROOT" "$PEP_MACOS_DT_APP_INFRA_NAME" "$PEP_MACOS_DT_APP_INSTALL_LOCATION" "$PEP_MACOS_APP_CONFIG_VERSION_LONG" "$PEP_MACOS_DT_APP_IDENTIFIER"

# New installer dirs
mkdir -p "installer/$PEP_MACOS_INSTALLER_PATH/update/$PEP_MACOS_ASS_APP_ROOT"
mkdir -p "installer/$PEP_MACOS_INSTALLER_PATH/update/$PEP_MACOS_CLI_APP_ROOT"
mkdir -p "installer/$PEP_MACOS_INSTALLER_PATH/update/$PEP_MACOS_DT_APP_ROOT"

# Move signed apps, to installer directory, to be used as update files
ditto "$PEP_MACOS_ASS_APP_ROOT/$PEP_MACOS_ASS_APP_INFRA_NAME.app" "installer/$PEP_MACOS_INSTALLER_PATH/update/$PEP_MACOS_ASS_APP_ROOT/$PEP_MACOS_ASS_APP_INFRA_NAME.app"
...
ditto "$PEP_MACOS_DT_APP_ROOT/$PEP_MACOS_DT_APP_INFRA_NAME.app" "installer/$PEP_MACOS_INSTALLER_PATH/update/$PEP_MACOS_DT_APP_ROOT/$PEP_MACOS_DT_APP_INFRA_NAME.app"
...
<installer-gui-script minSpecVersion="1">
    ...
    <choices-outline>
        <line choice="pepassessor"/>
        <line choice="pepcli"/>
        <line choice="download-tool"/>
    </choices-outline>
    <choice id="pepassessor" title="$PEP_MACOS_ASS_APP_NAME" description="$PEP_MACOS_ASS_APP_NAME">
        <pkg-ref id="$PEP_MACOS_ASS_APP_IDENTIFIER" version="$PEP_MACOS_APP_CONFIG_VERSION" onConclusion="none">$PEP_MACOS_ASS_APP_INFRA_NAME.pkg</pkg-ref>
    </choice>
    <choice id="pepcli" title="$PEP_MACOS_CLI_APP_NAME" description="$PEP_MACOS_CLI_APP_NAME">
    ...
    <choice id="download-tool" title="$PEP_MACOS_DT_APP_NAME" description="$PEP_MACOS_DT_APP_NAME">
        <pkg-ref id="$PEP_MACOS_DT_APP_IDENTIFIER" version="$PEP_MACOS_APP_CONFIG_VERSION" onConclusion="none">$PEP_MACOS_DT_APP_INFRA_NAME.pkg</pkg-ref>
    </choice>
...
rm -f "$PEP_MACOS_ASS_APP_INFRA_NAME.pkg"
rm -f "$PEP_MACOS_CLI_APP_INFRA_NAME.pkg"
rm -f "$PEP_MACOS_DT_APP_INFRA_NAME.pkg"
