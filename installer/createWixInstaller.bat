...
if "%buildnumber%" == "" (
	echo Usage: %0 ^<buildDir^> ^<wixLibPath^> ^<configDir^> ^<environment^> ^<buildNumber^>
	echo E.g. : %0 "C:\proj\bin" "C:\pepBinaries.wixlib" C:\proj\config acc 12345
	exit /B 1
)
...
pushd %OwnDir%
REM Specify %buildnumber% to ensure that configVersion.json has the same version as the installer
call "%OwnDir%\..\scripts\invoke-sh.bat" "../scripts/createConfigVersionJson.sh" "%unixinfradir%" "%unixprojectdir%" "%buildnumber%" > "%artifactsdir%\configVersion.json" || exit /B 1
popd
...
"%WIX%\bin\candle.exe" -nologo -dBuildNumber="%buildnumber%" -dProjectCaption="%projectcaption%" -dEnvironmentName="%environmentname%" -dInfraWxiFile="%infradir%\WindowsInstaller.wxi" -arch x64 -ext WixUtilExtension -out "%wixdir%\main.wixobj" "%OwnDir%\pep.wxs" || exit /B 1
