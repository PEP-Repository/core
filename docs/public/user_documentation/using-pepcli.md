...
- [`asa`](#asa) provides subcommands to perform administrative tasks related to authentication. Only for users enrolled as an `Access Administrator`:
  - [`asa query`](#asa-query) List users and user groups
  - [`asa user create/remove`](#asa-user-createremove) Create and remove users
  - [`asa user addIdentifier/removeIdentifier`](#asa-user-addidentifierremoveidentifier) Add and remove identifiers for users
  - [`asa user addTo/removeFrom`](#asa-user-addtoremovefrom) Add users to or remove users from user groups
  - [`asa group create/remove/modify`](#asa-group-createremovemodify) Create, remove or modify user groups
  - [`asa token request`](#asa-token-request) Generate a new authentication token for a user
  - [`asa token block create/remove/list`](#asa-token-block-create-remove-list) Blocking and unblocking tokens by managing a blocklist on the server.
...
The `asa` command's various sub-commands can be used to perform administrative tasks related to authentication. Users have
to be enrolled as `Access Administrator` in order to perform `asa` commands.

### `asa query`

The `asa query` subcommand lists all user-groups and users that are known to the authentication server.
...
This is an example of the invocation and output of `asa query`:
...
- Users have one or more user identifiers. See [`asa user addIdentifier/removeIdentifier`](#asa-user-addidentifierremoveidentifier)
  for more information about using multiple identifiers.

The result of `asa query` can be filtered with one of the following flags:
...
You can use `--format json` to format the output of `asa query` as json.

### `asa user create/remove`

New users can be created with the following command:

```shell
pepcli asa user create <uid>
```
...
### `asa user addIdentifier/removeIdentifier`
...
These different kinds of user identifiers are not handled differently by the `asa` commands. It is not stored in PEP if
...
```shell
pepcli asa user addIdentifier <existingUid> <newUid>
```
...
```shell
pepcli asa user removeIdentifier <uid>
```
...
the user entirely with the `pepcli asa user remove` command, if that is desired.

### `asa user addTo/removeFrom`

Users can be added to, and removed from user groups with the following commands:

```shell
pepcli asa user addTo <uid> <group>
pepcli asa user removeFrom <uid> <group>
```

### `asa group create/remove/modify`

User groups can be created and removed with the following commands:

```shell
pepcli asa group create <name>
pepcli asa group remove <name>
```

If users in a user group should be able to request tokens for themselves, you can create the group as follows:

```shell
pepcli asa group create --max-auth-validity <validity_period> <name>
```
...
```shell
pepcli asa group modify --max-auth-validity <validity_period> <name>
```

If you want to remove the ability for users of a user group to request tokens for themselves, you can use the `asa modify`
subcommand, with the `--max-auth-validity` left out:

```shell
pepcli asa group modify <name>
```

### asa token request

```shell
Usage: pepcli asa token request [--expiration-yyyymmdd <value>] [--json] <subject> <user-group> [expiration-unixtime]
```
...
- **passing a timestamp wit a 3rd positional argument, `expiration-unixtime` (advanced):**
...
> ℹ️ **Note:**
> The command usage notation we use here and in the `--help` text does not consider relations between parameters, so there
> both parameters are marked as optional, even though you are required to pass one of them.
...
> pepcli asa token request "U1234567@university-of-peppers.com" "Research Assessor" --expiration-yyyymmdd 20270322
iMTc1NjQ2[...]62buW1Sf
...
> pepcli asa token request --json "dataadmin@master.pep.cs.ru.nl" "Access Administrator" 1756461600
{
  "OAuthToken": "iMTc1NjQ2[...]CdI-IxNz"
}
```

### asa token block create, remove, list
...
All `pepcli asa token block` commands operate on this blocklist.
...
```shell
Usage: pepcli asa token block create [--issuedBefore-unixtime <value>] [--issuedBefore-yyyymmdd <value>] --message <value> <subject> <user-group>
```

The main command is `pepcli asa token block create`, which can block tokens by adding new rejection rules to the list.
...
> pepcli asa token block create "user894@some_domain.com" "Monitor" -m "Token was mailed to wrong address"
...
> pepcli asa token block create "U1234567@university-of-peppers.com" "Research Assessor" -m "User no longer active" --issuedBefore-yyyymmdd "20240101"
id, targetSubject, targetUserGroup, targetIssueDateTime, note, issuer, creationDateTime
...
```shell
Usage: pepcli asa token block list
```

Run `pepcli asa token block list` to get a list of all rejection rules that are currently in effect.
...
```shell
Usage: pepcli asa token block remove <id>
```

The `pepcli asa token block remove` can remove any blocklist entry that is currently in effect.
This essentially rolls back a previously entered `pepcli asa token block create`.

The command expects one parameter, namely the numeric asa token blocklist id of the entry that you wish to remove,
...
| `--metadata`                          | `-m`       | Print metadata. It may contain encrypted entries when only an ID was returned for the file in question; apply `pepcli get` to the ID to get the decrypted entries. |
