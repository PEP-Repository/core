...
| ------- | ------------ | ----------- |
| Access Administrator | AA | The PEP role responsible for [data access management](access-control.md#data-access) and [assignment of users to user groups](access-control.md#role-determination). Users with this role are called "access administrators". Not to be confused with the "Acccess Manager" service. |
...
| Access Ruless | AR | Rules describing access, read, and write permissions of **User Groups** to **Data Subject Groups** and **Column Groups**. |
| Access Version | | A version of the set of Access Rules a User Group has access to. This version is defined by a **Access Rules Timestamp**. The Access Version is maintained by the Access Manager. The Access Version also refers to a **Data Version** |
| Access Version Identifier | | (see **Version Names**) |
| Access Version Timestamp | | A timestamp that defines which **Access Rules** (which vary through time) are part of the **Access Version** |
...
| Data Version |  | The Data Version defines the set of **Cell Versions** a User Group may access within the allowed Access Rules, based on a **Data Version Timestamp** which decides what Cell Versions to use. The Data Version is referred to by the Access Version. The Data Version is managed by the Data Administrator. |
| Data Version Identifier | | (see **Version Names**) |
| Data Version Timestamp | | A timestamp used by the **Data Version** which defines which cross section of **Cell Versions** are part of the version |
...
| Version Names | | To improve readability, the **Access Version** and the **Data Version** are referred to by this identifying semantic (user defined) name, resp. the **Access Version Identifier** and the **Data Version Identifier** |
