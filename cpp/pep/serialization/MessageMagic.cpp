...
MessageMagic GetMessageMagic(const std::string_view& str) {
  ...
  return ntohl(*reinterpret_cast<const MessageMagic*>(str.data()));
}
...
void BasicMessageMagician::WriteMagicTo(std::ostream& destination, MessageMagic magic) {
  MessageMagic dwMagic = htonl(magic);
  destination.write(reinterpret_cast<char*>(&dwMagic), sizeof(MessageMagic));
}
