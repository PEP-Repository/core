...
#define PEP_DEFINE_PROTOCOL_BUFFER_SERIALIZATION(t, protot) \
  ...
  struct ProtocolBufferedSerialization<t> { \
    typedef protot ProtocolBufferType; \
  }; \
  \
  template <> \
  struct ProtocolBufferedDeserialization<protot> { \
    typedef t DeserializedType; \
  }
...
public:
  typedef typename ProtocolBufferedSerialization<T>::ProtocolBufferType ProtocolBufferType;
  ...
  template <template <class ...> class ContainerT, class ... Args>
  void copyToRepeatedProtocolBuffer(::google::protobuf::RepeatedPtrField<ProtocolBufferType>& destination, ContainerT<T, Args...> source) {
    destination.Clear();
    ...
    auto endsource = source.end();
    for (auto isource = source.begin(); isource != endsource; ++isource) {
      moveIntoProtocolBuffer(*destination.Add(), std::move(*isource));
    }
  }
};
...
protected:
  ProtocolBufferedEmptyMessageSerializer() noexcept {}
