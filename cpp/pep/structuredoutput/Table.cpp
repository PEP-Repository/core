...
Table::RecordRef Table::emplace_back(std::vector<std::string> record) {
  ...
  const auto lastRecordBegin = mData.end() - static_cast<long>(mRecordSize);
...
std::vector<Table::ConstRecordRef> Table::records() const noexcept {
  ...
  const auto stepSize = static_cast<long>(mRecordSize);
...
Table::RecordRef Table::asMutable(ConstRecordRef ref) {
  ...
  return {const_cast<decltype(mData.data())>(ref.data()), ref.size()}; // safe: this has access to non-const mData
}
