...
static const std::string LOG_TAG ("DumpShadowAdministration");
...
class DumpShadowAdministrationApplication : public pep::Application {
  ...
  protected:
    ...
    int execute() override {
      ...
      DumpShadowAdministration(database.string().c_str(), shadowPrivateKey);
      return EXIT_SUCCESS;
    }
  ...
  protected:
    ...
    int execute() override {
      ...
      CreateShadowAdministration(database.string().c_str(), shadowPublicKey, inputFile.string().c_str());
      return EXIT_SUCCESS;
    }
...
PEP_DEFINE_MAIN_FUNCTION(pep::DumpShadowAdministrationApplication)
