...
class ClientTestApplication : public Application {
  ...
  typedef std::function<rxcpp::observable<bool>(std::shared_ptr<Client>)> TestFunction;
  ...
  int runTestFunction(const TestFunction& function) {
    Configuration config = this->loadMainConfigFile(this->getParameterValues().get<std::filesystem::path>("config-file"));
    auto io_service = std::make_shared<boost::asio::io_service>();
    client = Client::OpenClient(config, io_service);
...
rxcpp::observable<bool> ClientTestApplication::Mode5Command::getTestResults(std::shared_ptr<Client> client) {
  ...
  ).map([](std::tuple<VersionResponse, std::string> response) {
    const BinaryVersion& version = std::get<0>(response).binary; // TODO: deal with config version as well
    const std::string& server = std::get<1>(response);
    std::cout << server
      << " version " << version.getSummary()
      << std::endl;

    auto ownPipelineId = BinaryVersion::current.getPipelineId();
    if ((ownPipelineId != version.getPipelineId()) && !ownPipelineId.empty() && !version.getPipelineId().empty()) {
      LOG(LOG_TAG, error) << server << " has pipeline ID " << version.getPipelineId() << " but expected pipeline ID " << ownPipelineId;
      return false;
    }
    return true;
...
PEP_DEFINE_MAIN_FUNCTION(pep::ClientTestApplication)
