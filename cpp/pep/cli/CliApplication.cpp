...
extern std::shared_ptr<ChildCommandOf<CliApplication>> CreateCommandList(CliApplication& parent);
extern std::shared_ptr<ChildCommandOf<CliApplication>> CreateCommandStore(CliApplication& parent);
extern std::shared_ptr<ChildCommandOf<CliApplication>> CreateCommandDelete(CliApplication& parent);
extern std::shared_ptr<ChildCommandOf<CliApplication>> CreateCommandGet(CliApplication& parent);
extern std::shared_ptr<ChildCommandOf<CliApplication>> CreateCommandPull(CliApplication& parent);
extern std::shared_ptr<ChildCommandOf<CliApplication>> CreateCommandAma(CliApplication& parent);
extern std::shared_ptr<ChildCommandOf<CliApplication>> CreateCommandAsa(CliApplication& parent);
extern std::shared_ptr<ChildCommandOf<CliApplication>> CreateCommandPing(CliApplication& parent);
extern std::shared_ptr<ChildCommandOf<CliApplication>> CreateCommandValidate(CliApplication& parent);
extern std::shared_ptr<ChildCommandOf<CliApplication>> CreateCommandVerifiers(CliApplication& parent);
extern std::shared_ptr<ChildCommandOf<CliApplication>> CreateCommandCastor(CliApplication& parent);
extern std::shared_ptr<ChildCommandOf<CliApplication>> CreateCommandMetrics(CliApplication& parent);
extern std::shared_ptr<ChildCommandOf<CliApplication>> CreateCommandRegister(CliApplication& parent);
extern std::shared_ptr<ChildCommandOf<CliApplication>> CreateCommandXEntry(CliApplication& parent);
extern std::shared_ptr<ChildCommandOf<CliApplication>> CreateCommandQuery(CliApplication& parent);
extern std::shared_ptr<ChildCommandOf<CliApplication>> CreateCommandHistory(CliApplication& parent);
extern std::shared_ptr<ChildCommandOf<CliApplication>> CreateCommandFileExtension(CliApplication& parent);
extern std::shared_ptr<ChildCommandOf<CliApplication>> CreateCommandTokenBlock(CliApplication& parent);
...
pep::commandline::Parameters CliApplication::getSupportedParameters() const {
    ...
    + pep::commandline::Parameter("client-working-directory", "Working directory to use").value(pep::commandline::Value<std::filesystem::path>()
      .directory().defaultsTo(pep::GetResourceWorkingDirForOS(".")))
    + pep::commandline::Parameter("client-config-name", "Name of client configuration file").value(pep::commandline::Value<std::filesystem::path>().defaultsTo("ClientConfig.json"))
    ...
    + pep::commandline::Parameter("oauth-token-group", "Group for generated token").value(pep::commandline::Value<std::string>().defaultsTo(pep::user_group::ResearchAssessor));
}
...
rxcpp::observable<pep::FakeVoid> CliApplication::connectClient(bool ensureEnrolled) {
  if (mClient != nullptr) {
    assert(mWork != nullptr);
    return rxcpp::observable<>::just(pep::FakeVoid());
  }

  const auto& parameterValues = this->getParameterValues();
  std::filesystem::path clientPath = parameterValues.get<std::filesystem::path>("client-working-directory");
  this->tryLoadConfigVersion(clientPath);

  auto configPath = parameterValues.get<std::filesystem::path>("client-config-name");
  if (!configPath.is_absolute()) {
    configPath = std::filesystem::absolute(clientPath / configPath);
  }
  pep::Configuration config = pep::Configuration::FromFile(configPath);
      ...
      if (tokenSecret.has_value()) {
        ...
        token = pep::OAuthToken::Generate(
          *tokenSecret,
          ...
          cmdlineGroup,
          now,
          now + parameterValues.get<uint64_t>("oauth-token-duration"));
  ...
  mWork = std::make_shared<boost::asio::io_service::work>(*io_service);
  mClient = client;
...
int CliApplication::executeEventLoopFor(bool ensureEnrolled, std::function<rxcpp::observable<pep::FakeVoid>(std::shared_ptr<pep::Client> client)> callback) {
  ...
  connectClient(ensureEnrolled)
    .flat_map([this, callback](pep::FakeVoid unused) {
      ...
      // io_service.run() usually returns when there is no work to do.
      // Our this->mWork prevents this though.
...
PEP_DEFINE_MAIN_FUNCTION(CliApplication)
