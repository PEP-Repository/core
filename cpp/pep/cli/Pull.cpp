...
struct Context {
  ...
  bool applyFileExtensions{ pep::DownloadDirectory::APPLY_FILE_EXTENSIONS_BY_DEFAULT };
  std::string outputDirectory;
  std::string tempDirectory;
  pep::DownloadDirectory::PullOptions options;
  pep::DownloadDirectory::ContentSpecification content;
...
void UpdateFormat(std::filesystem::path directory, std::shared_ptr<pep::GlobalConfiguration> globalConfig) {
  auto metadataDir = directory / pep::DownloadMetadata::GetDirectoryName();
...
* \param Shared pointer to the Context
*/
...
rxcpp::observable<std::shared_ptr<Context>> createContext(const std::shared_ptr<pep::CoreClient> client, const pep::commandline::NamedValues& values) {
  ...
  if (values.has("report-progress")) {
    ctx->progress->onChange(ReportProgress);
  }
...
* \param globalConfig Shared pointer to the GlobalConfiguration
*/
std::shared_ptr<pep::DownloadDirectory> createDownloadDirectory(const std::shared_ptr<Context> ctx, const std::shared_ptr<pep::CoreClient> client, const std::shared_ptr<pep::GlobalConfiguration> globalConfig, bool applyFileExtensions) {
  std::shared_ptr<pep::DownloadDirectory> directory;
  if (ctx->update) {
    prepareTempDirectory(ctx);
    directory = pep::DownloadDirectory::Create(ctx->tempDirectory, globalConfig);
    ...
    directory = pep::DownloadDirectory::Create(ctx->tempDirectory, client, ctx->content, globalConfig, applyFileExtensions);
  }
  return directory;
}
