...
const std::string SPECIFICATION_FILENAME = pep::DownloadMetadata::GetFilenamePrefix() + "specification" + pep::DownloadMetadata::GetFilenameExtension();
...
std::optional<DownloadDirectory::Specification> DownloadDirectory::tryReadSpecification() const {
  ...
  if (content) {
    return pep::DownloadDirectory::Specification::FromString(*content);
  }
  return std::nullopt;
}
...
template <>
class PropertySerializer<pep::DownloadDirectory::ContentSpecification> : public PropertySerializerByReference<pep::DownloadDirectory::ContentSpecification> {
public:
  void read(pep::DownloadDirectory::ContentSpecification& destination, const boost::property_tree::ptree& source, const MultiTypeTransform& transform) const override {
  ...
  void write(boost::property_tree::ptree& destination, const pep::DownloadDirectory::ContentSpecification& value) const override {
...
template <>
class PropertySerializer<pep::DownloadDirectory::Specification> : public PropertySerializerByReference<pep::DownloadDirectory::Specification> {
public:
  void read(pep::DownloadDirectory::Specification& destination, const boost::property_tree::ptree& source, const MultiTypeTransform& transform) const override {
  ...
  void write(boost::property_tree::ptree& destination, const pep::DownloadDirectory::Specification& value) const override {
...
std::shared_ptr<DownloadDirectory::RecordStorageStream> DownloadDirectory::create(const RecordDescriptor& descriptor, bool pseudonymisationRequired, bool archiveExtractionRequired, size_t fileSize) {
  remove(descriptor);

  assert(!getRecordFileName(descriptor));

  return std::shared_ptr<RecordStorageStream>(new RecordStorageStream(shared_from_this(), descriptor, this->getDataStoragePath(descriptor), pseudonymisationRequired, archiveExtractionRequired, fileSize));
}
...
DownloadDirectory::RecordStorageStream::RecordStorageStream(std::shared_ptr<DownloadDirectory> destination, const RecordDescriptor& descriptor, std::filesystem::path path, bool pseudonymisationRequired, bool archiveExtractionRequired, size_t fileSize)
  : mDestination(std::move(destination)), mDescriptor(descriptor), mPath(std::move(path)), mFileName(mPath.filename().string()), mFileSize(fileSize), mHasher(HashedArchive::DOWNLOAD_HASH_SEED), mPseudonymisationRequired(pseudonymisationRequired), mArchiveExtractingRequired(archiveExtractionRequired) {
...
DownloadDirectory::RecordStorageStream::~RecordStorageStream() noexcept {
  if (mRaw) {
    LOG(LOG_TAG, pep::error) << "Error destructing RecordStorageStream: uncommitted record at " + mPath.string(); // TODO: improve
  }
}
...
void DownloadDirectory::RecordStorageStream::commit(std::shared_ptr<GlobalConfiguration> globalConfig) {
  ...
  if (mArchiveExtractingRequired) {
    ...
    if (mPseudonymisationRequired) {
      ...
      pseudonymiser = pep::Pseudonymiser(mDescriptor.getExtra().at("pseudonymPlaceholder").plaintext(), localPseudonym);
    }
