...
protected:
  typedef std::map<std::string, std::string> ColumnExtensions;
  ...
  struct ParticipantSpecification {
    ...
    std::string str() const {
      ...
      case PARTICIPANT:
        ...
        break;
      case SHORT_PSEUDONYM:
        description = "Short pseudonym";
        break;
      default:
        throw std::runtime_error("Unsupported participant specification kind " + std::to_string(static_cast<std::underlying_type<Kind>::type>(kind)));
      }
      return description + " " + value;
    }
  };

  typedef std::optional<std::string> ParticipantSpecification::* ParticipantSpecificationStringField;
  typedef std::function<rxcpp::observable<MultiCellQuery::ParticipantSpecAndPp>(const pep::commandline::NamedValues&, std::shared_ptr<pep::CoreClient>)> SpecAndPpRetrievalFunction;
