    ...
    int execute() override {
      ...
      auto connectivityResult = this->executeEventLoopFor(
        ...
        return client->getGlobalConfiguration()
          ...
          .flat_map([](std::shared_ptr<std::unordered_map<uint32_t, ParticipantData>> data) { return rxcpp::observable<>::iterate(std::move(*data)); }) // Convert to an observable<key-value-pair>
