  ...
  commandline::Parameters getSupportedParameters() const override {
    return ChildCommandOf::getSupportedParameters()
        + commandline::Parameter("subject", "Name of the structure we should retrieve metadata for")
              .value(commandline::Value<std::string>().positional().required())
        + commandline::Parameter("key", "Metadata key we should retrieve (metadata_group:subkey or metadata_group:*)")
    ...
    return this->executeEventLoopFor([subjectType = getParent().getMetadataType(),
      ...
      return client->getStructureMetadata(subjectType, {requestedSubjectKey.subject}, {requestedSubjectKey.key})
          ...
          .map([requestedSubjectKey](const std::shared_ptr<std::vector<std::shared_ptr<StructureMetadataEntry>>>& entries) -> FakeVoid {
            ...
            auto setStdoutBinary = SetStdoutToBinaryInScope();
         ...
         values = this->getParameterValues()](const std::shared_ptr<CoreClient>& client) -> rxcpp::observable<FakeVoid> {
          ...
          if (json) {
            ...
            return getEntries.map([root](const std::shared_ptr<StructureMetadataEntry>& entry) -> FakeVoid {
              root->add(RawPtreePath(entry->subjectKey.subject) / RawPtreePath(entry->subjectKey.key.toString()), std::move(entry->value));
              return {};
  ...
  commandline::Parameters getSupportedParameters() const override {
    return ChildCommandOf::getSupportedParameters()
        ...
        + commandline::Parameter("key", "Metadata key we should set (metadata_group:subkey)")
              .value(commandline::Value<std::string>().required())
        + commandline::Parameter("value", "Metadata value").value(commandline::Value<std::string>());
  }
  ...
  int execute() override {
    return this->executeEventLoopFor([subjectType = getParent().getMetadataType(),
      ...
      if (!value) {
        ...
        auto setStdinBinary = SetStdinToBinaryInScope();
