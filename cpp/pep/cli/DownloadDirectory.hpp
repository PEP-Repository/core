...
public:
  ...
  private:
    explicit RecordStorageStream(std::shared_ptr<DownloadDirectory> destination, const RecordDescriptor& descriptor, std::filesystem::path path, bool pseudonymisationRequired, bool archiveExtractionRequired, size_t fileSize);
  ...
  struct PullOptions {
    ...
    inline PullOptions() noexcept {} // Workaround for Clang build failure: see https://stackoverflow.com/a/44693603
  };
...
public:
  virtual inline ~DownloadDirectory() noexcept {}
  ...
  std::shared_ptr<RecordStorageStream> create(const RecordDescriptor& descriptor, bool pseudonymisationRequired, bool archiveExtractionRequired, size_t fileSize);
