...
/// Returns true iff every file in column \p columnNr is smaller then \p sizeInBytes
bool AllColumnFilesAreSmaller(const Table& table, std::size_t columnNr, std::size_t sizeInBytes) {
  return AllOfFieldsInColumn(table, columnNr, [sizeInBytes](const std::string& field) {
    return field.empty() || std::filesystem::file_size(field) < sizeInBytes;
  });
}

/// Returns true iff every byte in the content of the file is a printable char
