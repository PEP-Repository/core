...
void DownloadMetadata::ensureFormatUpToDate() {
  ...
  if (std::filesystem::exists(legacyPristineFile)) {
    PEP_LOG(LogTag, warning) << "Upgrading legacy download directory format.";
    ...
    PEP_LOG(LogTag, warning) << "Download directory metadata format upgraded. Please update your (offline) copies.";
  }
