    ...
    return CreateContext(client, this->getParameterValues(), pp, column)
      .flat_map([client, opts, cleanupFiles](std::shared_ptr<StoreContext> context) {
        ...
        if (context->data.has_value()) {
          ...
          std::shared_ptr<std::optional<decltype(pep::SetStdinToBinaryInScope())>> setStdinBinary;
          if (context->pseudonym.has_value() || context->requiresDirectory) {
            ...
            if (path.string() == "-") {
              setStdinBinary = std::make_shared<decltype(setStdinBinary)::element_type>(pep::SetStdinToBinaryInScope());
          ...
          batches = pep::IStreamToMessageBatches(stream);
          if (setStdinBinary) {
