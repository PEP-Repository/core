...
rxcpp::observable<rxcpp::observable<std::shared_ptr<std::string>>> IStreamToDataGenerator(std::shared_ptr<std::istream> stream) {
  return pep::CreateObservable<rxcpp::observable<std::shared_ptr<std::string>>>([stream, first = std::make_shared<bool>(true)](rxcpp::subscriber<rxcpp::observable<std::shared_ptr<std::string>>> subscriber) {
    ...
    *first = false;
    ...
    const size_t CHUNK_SIZE = static_cast<size_t>(pep::CoreClient::DEFAULT_PAGE_SIZE);
    while (stream->good()) {
      // Create instance to store next page
      auto page = std::make_shared<std::string>();
      page->resize(CHUNK_SIZE);

      // Read data from stream into page
      stream->read(&(*page)[0], static_cast<std::streamsize>(CHUNK_SIZE));
...
void AddSpecifiedMetadata(std::map<std::string, pep::MetadataXEntry>& metadata, const pep::commandline::NamedValues& parameterValues) {
  ...
  for (auto&& json
    ...
    auto [it, inserted] = metadata.insert({
      e.name(),
      pep::Serialization::FromProtocolBuffer(
          std::move(*e.mutable_value()))
      });
    if (!inserted) {
      std::stringstream ss;
    ...
    return CreateContext(client, this->getParameterValues(), pp, column)
      .flat_map([client, opts, cleanupFiles](std::shared_ptr<StoreContext> context) {
        ...
        if (context->data.has_value()) {
          generator = rxcpp::observable<>::just(rxcpp::observable<>::just(std::make_shared<std::string>(*context->data)).as_dynamic());
        }
        else {
          ...
          if (context->pseudonym.has_value() || context->requiresDirectory) {
            ...
            if (path.string() == "-") {
              ...
              if (::_setmode(::_fileno(stdin), _O_BINARY) == -1) {
                std::stringstream ss;
                ss << "Could not set stdin to binary mode: " << std::strerror(errno);
                throw std::runtime_error(std::move(ss).str());
              }
#endif
          ...
          generator = IStreamToDataGenerator(stream);
        }

        pep::StoreData2Entry entry(context->pp, context->column, generator);
