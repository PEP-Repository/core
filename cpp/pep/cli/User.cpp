...
std::vector<std::shared_ptr<pep::commandline::Command>> CommandUser::createChildCommands() {
  return {
    ...
    std::make_shared<UserGroupUserSubCommand>("addTo", "Add user to a group", &pep::CoreClient::addUserToGroup, *this),
    std::make_shared<UserGroupUserSubCommand>("removeFrom", "Remove user from a group",  &pep::CoreClient::removeUserFromGroup, *this),
  };
}
