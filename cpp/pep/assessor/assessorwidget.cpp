...
void AssessorWidget::setAssessors(const std::vector<pep::AssessorDefinition>& assessors, const pep::StudyContext& studyContext) {
  ...
  assessors_ = assessors
    ...
    | to<std::vector>();
  sort(assessors_, {}, &pep::AssessorDefinition::name);
...
void AssessorWidget::setCurrentAssessor(const std::optional<unsigned int>& id) {
  ...
  if (id.has_value()) {
    auto position = std::ranges::find(assessors_, *id, &pep::AssessorDefinition::id); // Qualified: unqualified "find" would resolve to QWidget::find
