...
namespace {
  ...
  private:
    ...
    static unsigned long ParseBuildNumber(const std::optional<pep::GitlabVersion>& version, const std::string& (pep::GitlabVersion::* getValue)() const noexcept, const std::string& description);
  ...
  public:
    ...
    static std::shared_ptr<LocalInstaller> GetAvailable() {
      auto dir = pep::Application::GetExecutablePath().parent_path().string();
  ...
  bool PublishedInstaller::supersedesRunningVersion() const {
    ...
    auto currentPipeline = ParseBuildNumber(version, &pep::GitlabVersion::getPipelineId, "pipeline");
    auto currentJob = ParseBuildNumber(version, &pep::GitlabVersion::getJobId, "job");
    auto installerPipeline = this->getPipelineId();
    auto installerJob = this->getJobId();

    if (installerPipeline > currentPipeline) {
      return true;
    }
    if (installerPipeline == currentPipeline && installerJob > currentJob) {
      return true;
    }
    return false;
  }

  unsigned long PublishedInstaller::ParseBuildNumber(const std::optional<pep::GitlabVersion>& version, const std::string&(pep::GitlabVersion::*getValue)() const noexcept, const std::string& description) {
    if (version == std::nullopt) {
      LOG(LOG_TAG, pep::warning) << "Returning 0 as " << description << " of unset version.";
      return 0U;
    }
    auto value = ((*version).*getValue)(); // std::optional<> does not provide an (overloaded) operator ->*
    if (value.empty()) {
      LOG(LOG_TAG, pep::warning) << "Interpreting empty " << description << " ID as value 0. Are you running locally built software?";
      return 0U;
    }
    return stoul(value);
  }
