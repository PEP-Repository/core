...
class PepAssessorApplication : public pep::Application {
 private:
  ...
  void prepareForExecution(QApplication& application) {
    ...
    pep::Configuration config = this->loadMainConfigFile(this->getParameterValues().get<std::filesystem::path>("config"));
    ...
    auto loginWidget = new LoginWidget(pepClient->getIOservice(), projectConfig, config, pep::GetExecutablePath().parent_path());
 ...
 public:
  PepAssessorApplication() {
    ...
    pep::SetWorkingDirectory(GetWritableDirectory());
#endif
  }
 ...
 protected:
  ...
  pep::commandline::Parameters getSupportedParameters() const override {
    return pep::Application::getSupportedParameters()
      + pep::commandline::Parameter("config", "Path to client configuration file").value(pep::commandline::Value<std::filesystem::path>()
        .defaultsTo(pep::GetResourceWorkingDirForOS("ClientConfig.json")));
  }
