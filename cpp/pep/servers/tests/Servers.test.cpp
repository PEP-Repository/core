...
protected:
  ...
  static std::shared_ptr<boost::asio::io_service> mIoService;
  static std::shared_ptr<pep::IOServiceThread> mIoServiceThread;
  static bool mKeepRunning;

  static void SetUpTestSuite() {
    if (mClient == nullptr) {
      mIoService = std::make_shared<boost::asio::io_service>();
      mIoServiceThread = std::make_shared<pep::IOServiceThread>(mIoService, &mKeepRunning);
      mClient = pep::Client::OpenClient(pep::Configuration::FromFile("ClientConfig.json"), mIoService, false);
...
std::shared_ptr<boost::asio::io_service> ServersTest::mIoService = nullptr;
std::shared_ptr<pep::IOServiceThread> ServersTest::mIoServiceThread = nullptr;
