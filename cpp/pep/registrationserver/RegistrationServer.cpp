...
RegistrationServer::Parameters::Parameters(std::shared_ptr<boost::asio::io_service> io_service, const Configuration& config)
  ...
  setShadowPublicKey(ReadFile(shadowPublicKeyFile));
  ...
  clientBuilder.setIOservice(getIOservice())
  ...
  if (!castorAPIKeyFile) {
    ...
    {
      auto castor = castor::CastorConnection::Create(*castorAPIKeyFile, this->getIOservice());
...
/*!
  * \param castorClient The Castor client connection
  * \return The Parameters instance itself
  */
...
void RegistrationServer::Parameters::check() {
  if(!client)
...
rxcpp::observable<std::string> RegistrationServer::initPseudonymStorage(const std::filesystem::path& shadowStorageFile) {
  ...
  return RxEnsureProgress(*pClient->getIOservice(), "Pseudonym storage initialization", [this, shadowStorageFile](std::shared_ptr<ActivityMonitor> monitor) {
    ...
    return getShortPseudonymDefinitions()
      ...
      .tap([](const EnumerateAndRetrieveResult& earResult) { // Ensure we have each pseudonym's value
      if (!earResult.mDataSet) { // TODO: support this
        throw std::runtime_error("Storage Facility did not return pseudonym as inline data for column " + earResult.mColumn + " and participant " + earResult.mLocalPseudonyms->mPolymorphic.text());
      }
    })
      ...
      return rxcpp::observable<>::iterate(*pps);
    })
...
RegistrationServer::RegistrationServer(std::shared_ptr<Parameters> parameters)
  ...
  mGlobalConfiguration(CreateRxCache([client = pClient]() {return RxEnsureProgress(*client->getIOservice(), "Global configuration retrieval", client->getGlobalConfiguration()); })),
  ...
  , mCastorStudies(CreateRxCache([io_service = pClient->getIOservice(), connection = mCastorConnection, config = mGlobalConfiguration]() -> rxcpp::observable<std::shared_ptr<castor::Study>> {
  ...
  return RxEnsureProgress(*io_service, "Castor study loading", LoadCastorStudies(connection->getStudies(), GetShortPseudonymDefinitions(config))); }))
#endif
{
...
/*!
  ...
  * \return The return code from the SQLite3 storage operation. This is SQLITE_OK if storage was successful
  */
...
rxcpp::observable<ShortPseudonymDefinition> RegistrationServer::getShortPseudonymDefinitions() const {
  return RxEnsureProgress(*pClient->getIOservice(), "Short pseudonym definition retrieval", GetShortPseudonymDefinitions(mGlobalConfiguration));
}
...
rxcpp::observable<rxcpp::observable<std::shared_ptr<std::string>>> RegistrationServer::handleSignedPEPIdRegistrationRequest(std::shared_ptr<SignedPEPIdRegistrationRequest> signedRequest) {
  signedRequest->open(this->getRootCAs()); // Checks if request is valid.
...
rxcpp::observable<rxcpp::observable<std::shared_ptr<std::string>>> RegistrationServer::handleSignedRegistrationRequest(std::shared_ptr<SignedRegistrationRequest> signedRequest) {
...
rxcpp::observable<rxcpp::observable<std::shared_ptr<std::string>>> RegistrationServer::handleListCastorImportColumnsRequest(std::shared_ptr<ListCastorImportColumnsRequest> lpRequest) {
#ifndef WITH_CASTOR
