...
RegistrationRequest Serializer<RegistrationRequest>::fromProtocolBuffer(proto::RegistrationRequest&& source) const {
  ...
  result.polymorphicPseudonym = PolymorphicPseudonym(Serialization::FromProtocolBuffer(std::move(*source.mutable_polymorph_pseudonym())));
  result.encryptedIdentifier = std::move(*source.mutable_encrypted_identifier());
  result.encryptionPublicKeyPem = std::move(*source.mutable_encryption_public_key_pem());
  return result;
}
...
void Serializer<RegistrationRequest>::moveIntoProtocolBuffer(proto::RegistrationRequest& dest, RegistrationRequest value) const {
  Serialization::MoveIntoProtocolBuffer(*dest.mutable_polymorph_pseudonym(), value.polymorphicPseudonym.getValidElgamalEncryption());
  *dest.mutable_encrypted_identifier() = std::move(value.encryptedIdentifier);
  *dest.mutable_encryption_public_key_pem() = std::move(value.encryptionPublicKeyPem);
}
...
ListCastorImportColumnsRequest Serializer<ListCastorImportColumnsRequest>::fromProtocolBuffer(proto::ListCastorImportColumnsRequest&& source) const {
  ...
  result.spColumn = std::move(*source.mutable_sp_column());
  result.answerSetCount = source.answer_set_count();
...
void Serializer<ListCastorImportColumnsRequest>::moveIntoProtocolBuffer(proto::ListCastorImportColumnsRequest& dest, ListCastorImportColumnsRequest value) const {
  *dest.mutable_sp_column() = std::move(value.spColumn);
  dest.set_answer_set_count(value.answerSetCount);
}
...
void Serializer<ListCastorImportColumnsResponse>::moveIntoProtocolBuffer(proto::ListCastorImportColumnsResponse& dest, ListCastorImportColumnsResponse value) const {
  dest.mutable_import_columns()->Reserve(static_cast<int>(value.importColumns.size()));
  for (auto& name : value.importColumns)
