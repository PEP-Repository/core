...
typedef std::pair<const std::string, MetadataXEntry> NamedMetadataXEntry;
...
class Metadata {
 public:
  ...
  inline Metadata(std::string tag, Timestamp date)
    : mBlindingTimestamp(std::move(date)), mTag(std::move(tag)) { }
  inline Metadata(std::string tag, Timestamp date, EncryptionScheme scheme)
    : mBlindingTimestamp(std::move(date)), mTag(std::move(tag)), mEncryptionScheme(scheme) { }
