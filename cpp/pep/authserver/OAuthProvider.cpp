...
rxcpp::observable<HTTPResponse> OAuthProvider::handleAuthorizationRequest(HTTPRequest request) {
  ...
  return authserverBackend->findUserGroupsAndStorePrimaryIdIfMissing(primaryUid, alternativeUids)
    ...
    if(groups->size() > 1) {
      ...
      if(groupQuery != formData.end()) {
        ...
        if(std::ranges::find_if(*groups, [&selectedGroup](const UserGroup& group){ return group.mName == selectedGroup; }) == groups->end()) {
