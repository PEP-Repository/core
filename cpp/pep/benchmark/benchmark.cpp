...
static pep::EncryptionKeyRequest createTestEncryptionKeyRequest() {
...
static void BM_KeyRequestSerialize(benchmark::State& state) {
  auto req = createTestEncryptionKeyRequest();
  for (auto _ : state)
...
static void BM_SignDigest(benchmark::State& state) {
  ...
  keypair.generateKeyPair();
  for (auto _ : state)
    benchmark::DoNotOptimize(keypair.getPrivateKey().signDigest(SAMPLE_SHA256_DIGEST));
}
...
static void BM_VerifyDigest(benchmark::State& state) {
  pep::AsymmetricKeyPair keypair;
  keypair.generateKeyPair();
  auto sig = keypair.getPrivateKey().signDigest(SAMPLE_SHA256_DIGEST);
  for (auto _ : state)
    benchmark::DoNotOptimize(keypair.getPublicKey().verifyDigest(SAMPLE_SHA256_DIGEST, sig));
}
