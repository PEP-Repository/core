...
CoreClient::enumerateAndRetrieveData2(const enumerateAndRetrieveData2Opts& opts) {
  ...
  typedef std::vector<std::shared_ptr<DataPayloadPage>> Pages;
  typedef std::unordered_map<uint32_t, std::shared_ptr<Pages>> IndexedPages;
  struct Context {
    bool includeData;
    ...
    [this, ctx](rxcpp::subscriber<EnumerateAndRetrieveResult> subscriber) {
      ...
      this->requestTicket2(*ctx->requestTicketOpts) // Get (indexed) ticket
        ...
        .flat_map([this, ctx](const IndexedTicket2& indexedTicket) {
          ...
          // ctx->pseudonyms is a copy of the mPseudonyms in the ticket, to prevent
          // too many copies.
          ctx->pseudonyms.reserve(ctx->ticket->mPseudonyms.size());
          std::transform(ctx->ticket->mPseudonyms.cbegin(), ctx->ticket->mPseudonyms.cend(),
                         std::back_inserter(ctx->pseudonyms),
                         [](const LocalPseudonyms& lps) { return MakeSharedCopy(lps); });

          if (ctx->requestTicketOpts->includeAccessGroupPseudonyms) {
            ctx->agPseuds.reserve(ctx->pseudonyms.size());
            std::transform(ctx->pseudonyms.cbegin(), ctx->pseudonyms.cend(), std::back_inserter(ctx->agPseuds),
                           [this](const std::shared_ptr<LocalPseudonyms>& p) {
                             if (!p->mAccessGroup)
                               throw std::runtime_error("AccessGroup pseudonym not set");
                             return MakeSharedCopy(p->mAccessGroup->decrypt(privateKeyPseudonyms));
                           });
          }
          ...
          return clientStorageFacility->sendRequest(std::make_shared<std::string>(Serialization::ToString(
              ...
              [ctx](std::shared_ptr<std::vector<DataEnumerationEntry2>> entriesWithData, std::string rawResponse) {
                ...
                for (auto& entry: response.mEntries) {
                  if (ctx->includeData && (ctx->dataSizeLimit == 0U || entry.mFileSize <= ctx->dataSizeLimit)) {
                    ...
                    res.mLocalPseudonyms = ctx->pseudonyms.at(entry.mPseudonymIndex);
                    if (!ctx->agPseuds.empty()) {
                      res.mAccessGroupPseudonym = ctx->agPseuds.at(entry.mPseudonymIndex);
                    }
            ...
            .flat_map([this, ctx](
              ...
              rxcpp::observable<FakeVoid> getKeys = this->unblindAndDecryptKeys(
                  convertDataEnumerationEntries(*entries, *ctx->signedTicket), ctx->signedTicket)
