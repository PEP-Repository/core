...
const uint64_t CoreClient::DEFAULT_PAGE_SIZE = 1024 * 1024 / 2; //To make sure it will fit within the reduced MAX_SIZE_OF_MESSAGE for debug builds
#else
const uint64_t CoreClient::DEFAULT_PAGE_SIZE = 1024 * 1024;
#endif

CoreClient::CoreClient(const Builder& builder):
  io_service(builder.getIOservice()), keysFilePath(builder.getKeysFilePath()),
...
std::shared_ptr<CoreClient> CoreClient::OpenClient(const Configuration& config,
  ...
  Builder builder;
  builder.initialize(config, io_service, persistKeysFile);
...
void CoreClient::Builder::initialize(
  ...
  try {
    ...
    try {
      ...
      keysFile = GetWorkingDirectory() / config.get<std::string>("KeysFile");
    ...
    if (io_service == nullptr) {
      this->setIOservice(std::make_shared<boost::asio::io_service>());

      IOServiceThread t(this->getIOservice());
      t.detach();
    } else {
...
std::shared_ptr<CoreClient::ServerConnection> CoreClient::tryConnectTo(const EndPoint& endPoint) {
  return CoreClient::ServerConnection::TryCreate(io_service, endPoint, caCertFilepath);
}
...
rxcpp::observable<SignedPingResponse> CoreClient::pingSigningServer(std::shared_ptr<ServerConnection> connection) const {
...
rxcpp::observable<VersionResponse> CoreClient::tryGetServerVersion(std::shared_ptr<CoreClient::ServerConnection> connection)
    const {
...
const std::shared_ptr<boost::asio::io_service>& CoreClient::getIOservice() const {
  return io_service;
}
