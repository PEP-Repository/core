...
rxcpp::observable<DataStorageResult2> CoreClient::storeData2(
  ...
  struct Context {
    ...
    std::vector<rxcpp::observable<rxcpp::observable<std::shared_ptr<std::string>>>> data;
  ...
  for (size_t i=0; i<entries.size(); i++) {
    ...
    ctx->data.push_back(entry.mDataGenerator);
  }
  ...
  .flat_map([this,ctx, requestedPps](IndexedTicket2 indexedTicket) {
    ...
    auto pages = CreateObservable<rxcpp::observable<rxcpp::observable<std::shared_ptr<std::string>>>>([ctx](rxcpp::subscriber<rxcpp::observable<rxcpp::observable<std::shared_ptr<std::string>>>> subscriber) {
      for (size_t i = 0; i < ctx->request->mEntries.size(); ++i) {
        subscriber.on_next(ctx->data[i].map([i, ctx, fileContext = std::make_shared<uint64_t>()](rxcpp::observable<std::shared_ptr<std::string>> obs) -> rxcpp::observable<std::shared_ptr<std::string>>{
...
rxcpp::observable<DataStorageResult2> CoreClient::updateMetadata2(
  ...
  return requestTicket2(ticketRequest) // Request a ticket
    ...
    return this->clientStorageFacility->sendRequest(MakeSharedCopy(Serialization::ToString(sign(enumRequest))))
      ...
      return this->unblindAndDecryptKeys(convertDataEnumerationEntries(*enumEntries, *signedTicket), signedTicket)
