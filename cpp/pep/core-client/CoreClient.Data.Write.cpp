...
rxcpp::observable<DataStorageResult2> CoreClient::storeData2(
  ...
  struct Context {
    ...
    std::vector<MessageBatches> data;
  ...
  .flat_map([this,ctx, requestedPps](IndexedTicket2 indexedTicket) {
    ...
    auto pages = CreateObservable<MessageBatches>([ctx](rxcpp::subscriber<MessageBatches> subscriber) {
      for (size_t i = 0; i < ctx->request->mEntries.size(); ++i) {
        subscriber.on_next(ctx->data[i].map([i, ctx, fileContext = std::make_shared<uint64_t>()](MessageSequence obs) -> MessageSequence {
