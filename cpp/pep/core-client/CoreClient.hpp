#pragma once
/* The "CoreClient" class provides an interface to interact with (servers in) a PEP system.
 * Its network connectivity is provided by the nested "ServerConnection" class, which is
 * defined in a separate header and considered an implementation detail.
 * This allows the "CoreClient" class to be used without be(com)ing dependent on our networking code.
 */
...
struct StoreData2Entry : public StoreMetadata2Entry {
  StoreData2Entry(
    std::shared_ptr<PolymorphicPseudonym> pp,
    std::string column,
    rxcpp::observable<rxcpp::observable<std::shared_ptr<std::string>>> generator) :
  ...
  rxcpp::observable<rxcpp::observable<std::shared_ptr<std::string>>> mDataGenerator;
};
...
class CoreClient : boost::noncopyable {
 public:
  static const uint64_t DEFAULT_PAGE_SIZE;
  static constexpr size_t DATA_RETRIEVAL_BATCH_SIZE{4000};
  class ServerConnection; // Public so that utility functions can access it, but the implementation is private/local to CoreClient and its helpers
 ...
 private:
  ...
  std::shared_ptr<ServerConnection> clientAccessManager;
  std::shared_ptr<ServerConnection> clientStorageFacility;
  std::shared_ptr<ServerConnection> clientTranscryptor;
 ...
 public:
  class Builder {
   public:
    Builder& setIOservice(std::shared_ptr<boost::asio::io_service> io_service) {
...
protected:
  ...
  std::shared_ptr<ServerConnection> tryConnectTo(const EndPoint& endPoint);
  rxcpp::observable<VersionResponse> tryGetServerVersion(std::shared_ptr<ServerConnection> connection) const;
  rxcpp::observable<SignedPingResponse> pingSigningServer(std::shared_ptr<ServerConnection> connection) const;
...
public:
 ...
 private:
  ...
  std::vector<EnumerateResult> convertDataEnumerationEntries(
    ...
    const SignedTicket2& ticket) const;
