...
std::shared_ptr<CoreClient::ServerConnection> EnsureConnected(std::shared_ptr<CoreClient::ServerConnection> serverConnection, const std::string& serverName) {
  if (!serverConnection) {
...
rxcpp::observable<FakeVoid> CoreClient::removeUserFromGroup(std::string uid, std::string group) {
  UserMutationRequest request;
