...
std::shared_ptr<Logging::Sink> ConsoleLogging::registerSink() const {
  return CreateSinkWrapper(
    boost::log::add_console_log(
                std::clog,
                boost::log::keywords::format = (boost::log::expressions::stream << boost::log::expressions::format_date_time< boost::posix_time::ptime >("TimeStamp", "%Y-%m-%d %H:%M:%S") << ": <" << severity << "> [" << channel << "] " << boost::log::expressions::smessage),
...
typedef boost::log::sinks::syslog_backend SystemLogBackend;
...
typedef boost::log::sinks::simple_event_log_backend SystemLogBackend;

boost::shared_ptr<SystemLogBackend> createSyslogBackend(const std::string& szHostname, unsigned short wPort) {
...
std::shared_ptr<Logging::Sink> SysLogging::registerSink() const {
  try {
    typedef boost::log::sinks::synchronous_sink<SystemLogBackend> sink_t;
