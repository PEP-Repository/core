...
private:
  typedef decltype(TRegistrar::template RegisterType<TDerived>()) RegistrationId;
...
public:
  ...
  virtual ~SelfRegistering() noexcept {
    static_assert(std::is_base_of<SelfRegistering, TDerived>::value, "The class specified as TDerived must inherit from this class");
