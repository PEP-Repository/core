...
public:
  ...
  SetFileMode(FILE* file, std::streambuf* stream, int mode) : file_{file}, stream_{stream} {
    prevMode_ = setMode(mode);
  }

  SetFileMode(const SetFileMode&) = delete;
  SetFileMode& operator=(const SetFileMode&) = delete;
  SetFileMode(SetFileMode&& other) noexcept
      : file_{std::exchange(other.file_, {})}, prevMode_{other.prevMode_} {}
  SetFileMode& operator=(SetFileMode&&) = delete;

  ~SetFileMode() noexcept { revert(); }
};
