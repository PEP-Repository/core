...
private:
  typedef PropertySerializerByReference<std::vector<TValue>> Base;
...
public:
  typedef std::unordered_map<std::string, TValue> Map;

private:
  typedef PropertySerializerByReference<Map> Base;
...
private:
  typedef PropertySerializerByReference<std::optional<TValue>> Base;
...
public:
  ...
  void read(std::optional<TValue>& destination, const boost::property_tree::ptree& source, const MultiTypeTransform& transform) const override {
    ...
    try {
      ...
      // Couldn't deserialize as string: this wasn't a JSON null value or "null" string
    }
    if (content) {
      if (*content == "null") {
