...
void Terminate(std::exception_ptr source) noexcept {
  auto message = GetExceptionMessage(std::move(source));
  LOG("Terminate", critical) << "Fatal exception occurred" << message;
  std::terminate();
}
