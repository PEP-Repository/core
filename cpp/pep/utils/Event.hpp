...
public:
  virtual ~EventContract() noexcept {}
...
class Event final : boost::noncopyable {
  ...
  // Handlers are invoked with "std::forward<TArgs>(args)..." to ensure that values are passed by value or reference as
  // specified by TArgs. Rvalue references would therefore also be std::forward<>ed to the handlers, causing multiple
  // handlers to receive an std::forward<> of the same rvalue reference. This would cause the second and later handlers
  // to receive copies of an already-moved-from value.
...
public:
   ...
   */
  Event() noexcept(noexcept(std::vector<std::shared_ptr<Contract>>()) && noexcept(std::vector<EventSubscription>())) {}
...
bool Event<TOwner, TArgs...>::Contract::notify(TArgs&&... args) {
  ...
  (*invokable)(std::forward<TArgs>(args)...);
...
void Event<TOwner, TArgs...>::notify(TArgs... args) const {
  ...
  for (const auto& contract : mContracts) {
    if (!contract->notify(std::forward<TArgs>(args)...)) { // ... and if the contract has been cancelled...
