...
std::string ReadFile(const char* path) {
  ...
  in.read(&content[0], static_cast<std::streamsize>(size)); // will throw on failures since we added failbit to the exception mask
  in.close();
  return content;
}
...
void IstreamToDestination(std::istream& in, std::function<void(const char * c, const std::streamsize l)> writeToDestination) {
  while (!in.eof()) {
    if (in.fail()) {
