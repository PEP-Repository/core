...
int SetFileMode::setMode(int mode) const {
  assert(file_ && stream_);
  // Flush / sync
  if (stream_->pubsync() == -1) {
    throw std::runtime_error("Could not sync stream");
  }
  if (auto ret = ::_setmode(_fileno(file_), mode); ret != -1) {
...
void SetFileMode::revert() noexcept {
  if (!file_) { return; }
  try {
    setMode(prevMode_);
    file_ = {};
  } catch (const std::system_error& ex) {
    LOG(LOG_TAG, warning) << ex.what();
  }
}
