...
const std::set<LifeCycler::Status>& GetAllowedLifeCycleTransitions(LifeCycler::Status from) {
  ...
  if (result == LIFECYCLE_STATUS_CHANGES.cend()) {
    throw std::runtime_error("Unsupported life cycle status " + std::to_string(LifeCycler::StatusValue(from)));
  }
...
std::underlying_type_t<LifeCycler::Status> LifeCycler::StatusValue(Status status) {
  return static_cast<std::underlying_type_t<LifeCycler::Status>>(status);
}
...
LifeCycler::Status LifeCycler::setStatus(Status status) {
  ...
  if (status != result) {
    if (result == Status::initialized && status == Status::initializing) { // When initialized instances call this->setStatus(Status::initializing) ...
      ...
      throw std::runtime_error("Can't transition from life cycle status " + std::to_string(LifeCycler::StatusValue(result)) + " to " + std::to_string(LifeCycler::StatusValue(status)));
    }
