...
typedef std::shared_ptr<const boost::property_tree::ptree> JsonPtr;
...
private:
  CastorConnection(const std::filesystem::path& apiKeyFile, std::shared_ptr<boost::asio::io_service> io_service);
  CastorConnection(const EndPoint& endPoint, const ApiKey& apiKey, std::shared_ptr<boost::asio::io_service> io_service, const std::optional<std::filesystem::path>& caCert = std::nullopt);
