...
typedef FakeCastorTest CastorClientTest;
...
TEST_F(CastorClientTest, RateLimited) {
  //First, make the FakeCastorApi return a "Too Many Requests" response, telling the client to retry after 3 seconds
  auto after = Timestamp::FromTimeT(std::time(nullptr) + 3).toString();
  ...
  //Then, update the resonse after 2 seconds
  auto updateResponseObservable = rxcpp::rxs::timer(std::chrono::seconds(2)).map([opts = options](auto) -> JsonPtr {
