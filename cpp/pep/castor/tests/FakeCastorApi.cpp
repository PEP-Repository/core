...
void FakeCastorApi::Connection::handleError(std::exception_ptr error) {
  PEP_LOG(LOG_TAG, info) << GetExceptionMessage(error);
...
void FakeCastorApi::Connection::handleReadHeaders(const networking::DelimitedTransfer::Result& result) {
  ...
  while (std::getline(responseStream, header) && header != "\r") {
    ...
    if(index != std::string::npos) {
      ...
      PEP_LOG(LOG_TAG, warning) << "Ignoring malformed header: " << header << '\n';
    }
  }
...
void FakeCastorApi::Connection::handleRequest() {
  PEP_LOG(LOG_TAG, debug) << "Received request: " << mMethod << " " << mPath << std::endl << mBody;
...
void FakeCastorApi::Connection::handleWrite(const networking::SizedTransfer::Result& result) {
  if (!result) {
    PEP_LOG(LOG_TAG, warning) << "Error while writing response: " << GetExceptionMessage(result.exception());
    return;
  }

  PEP_LOG(LOG_TAG, debug) << "Written output " << mOutput.substr(0, *result);
}
...
FakeCastorTest::FakeCastorTest()
  ...
  mClientSide.start();
  mServerSide.start();
  PEP_LOG(LOG_TAG, info) << "FakeCastorApi listening on port " << port;
}
...
void FakeCastorApi::start() {
  mConnectivityConnectionAttempt = mConnectivity->onConnectionAttempt.subscribe([self = SharedFrom(*this)](const networking::Connection::Attempt::Result& result) {
    if (!*result) {
      PEP_LOG(LOG_TAG, warning) << "Incoming Fake Castor API connection failed: " << GetExceptionMessage(result.exception());
    }
    else {
