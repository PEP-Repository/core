...
void FakeCastorApi::Connection::handleReadBody(const boost::system::error_code& error, size_t bytes) {
  ...
  responseStream.read(body.data(), static_cast<std::streamsize>(body.size()));
...
void FakeCastorApi::Connection::writeOutput(const std::string& body, const std::string& status, std::map<std::string, std::string> responseHeaders) {
  ...
  output += "\r\n";
  output += body;
  boost::asio::async_write(*socket, boost::asio::buffer(output.data(), output.length()),
...
FakeCastorTest::FakeCastorTest()
  : io_context(std::make_shared<boost::asio::io_context>()), parameters(std::make_shared<FakeCastorApi::Parameters>()) {
  parameters->setIoContext(std::make_shared<boost::asio::io_context>());
  ...
  io_context_thread = IoContextThread(io_context, &runIoContext);
  fakeCastorThread = IoContextThread(parameters->getIoContext(), &runFakeCastor);
