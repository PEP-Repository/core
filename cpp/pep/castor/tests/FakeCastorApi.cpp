...
void FakeCastorApi::Connection::handleError(std::exception_ptr error) {
  PEP_LOG(LogTag, info) << GetExceptionMessage(error);
...
void FakeCastorApi::Connection::handleRequest() {
  PEP_LOG(LogTag, debug) << "Received request: " << mMethod << " " << mPath << std::endl << mBody;
...
void FakeCastorApi::Connection::handleWrite(const networking::SizedTransfer::Result& result) {
  if (!result) {
    PEP_LOG(LogTag, warning) << "Error while writing response: " << GetExceptionMessage(result.exception());
    return;
  }

  PEP_LOG(LogTag, debug) << "Written output " << mOutput.substr(0, *result);
}
...
FakeCastorTest::FakeCastorTest()
  ...
  mClientSide.start();
  mServerSide.start();
  PEP_LOG(LogTag, info) << "FakeCastorApi listening on port " << port;
}
...
void FakeCastorApi::start() {
  mConnectivityConnectionAttempt = mConnectivity->onConnectionAttempt.subscribe([self = SharedFrom(*this)](const networking::Connection::Attempt::Result& result) {
    if (!*result) {
      PEP_LOG(LogTag, warning) << "Incoming Fake Castor API connection failed: " << GetExceptionMessage(result.exception());
    }
    else {
