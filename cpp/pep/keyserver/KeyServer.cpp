...
KeyServer::Parameters::Parameters(std::shared_ptr<boost::asio::io_context> io_context, const Configuration& config)
  : Server::Parameters(io_context, config) {
...
MessageBatches KeyServer::handleUserEnrollmentRequest(
...
MessageBatches KeyServer::handleTokenBlockingListRequest(
...
MessageBatches KeyServer::handleTokenBlockingCreateRequest(
  ...
  return BatchSingleMessage(TokenBlockingCreateResponse{std::move(entry)});
}

MessageBatches KeyServer::handleTokenBlockingRemoveRequest(
  ...
  return BatchSingleMessage(TokenBlockingRemoveResponse{std::move(*entry)});
}
