...
protected:
  ...
  pep::commandline::Parameters getSupportedParameters() const override {
    return pep::Application::getSupportedParameters()
      ...
      + pep::commandline::Parameter("config-file", "Path to client configuration file").value(pep::commandline::Value<std::filesystem::path>().positional().required());
  }

  int execute() override {
    const auto& values = this->getParameterValues();
    auto configPath = std::filesystem::absolute(values.get<std::filesystem::path>("config-file"));
    pep::ConfigVersion::TryLoad(configPath.parent_path());
    ...
    return pep::castor::EnvironmentPuller::Pull(configPath, values.has("dry"), spColumns, sps)
      ? EXIT_SUCCESS
      : EXIT_FAILURE;
  }
};
