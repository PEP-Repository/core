...
private:
  class FormPuller;
  typedef std::vector<std::shared_ptr<FieldValue>> FieldValues;
  typedef std::unordered_map<std::string, std::shared_ptr<FormPuller>> FormPullersByFormId;

  typedef std::vector<std::shared_ptr<StudyDataPoint>> StudyDataPoints;
  typedef std::unordered_map<std::shared_ptr<Participant>, std::shared_ptr<StudyDataPoints>> StudyDataPointsByParticipant;
