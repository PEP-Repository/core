...
private:
  typedef std::unordered_map<std::string, std::shared_ptr<Study>> StudiesBySlug;
  ...
  EnvironmentPuller(std::shared_ptr<boost::asio::io_service> ioService, const std::filesystem::path& configFile, bool dry, const std::optional<std::vector<std::string>>& spColumns, const std::optional<std::vector<std::string>>& sps);
  ...
  * \return An observable emitting a Client instance that has been enrolled with the PEP system.
  */
  rxcpp::observable<std::shared_ptr<Client>> getClient();
...
public: // To be called from pepPullCastor('s main function)
  ...
  * \param configFile The path to a PEP (Client)Config.json file
  ...
  */
  static bool Pull(const std::filesystem::path& configFile, bool dry, const std::optional<std::vector<std::string>>& spColumns, const std::optional<std::vector<std::string>>& sps);
