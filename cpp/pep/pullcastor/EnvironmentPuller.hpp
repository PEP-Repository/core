...
private:
  ...
  EnvironmentPuller(std::shared_ptr<boost::asio::io_service> ioService, const Configuration& config, bool dry, const std::optional<std::vector<std::string>>& spColumns, const std::optional<std::vector<std::string>>& sps);
...
public: // To be called from pepPullCastor('s main function)
  ...
  * \param configFile The contents of the PEP (Client)Config.json file
