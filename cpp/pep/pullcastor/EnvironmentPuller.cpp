...
EnvironmentPuller::EnvironmentPuller(std::shared_ptr<boost::asio::io_context> io_context, const Configuration& config, bool dry, const std::optional<std::vector<std::string>>& spColumns, const std::optional<std::vector<std::string>>& sps)
  ...
  try {
    ...
    PEP_PULLCASTOR_LOG(critical) << "Error with PullCastor configuration file: " << e.what();
    throw;
  }
  ...
  try {
    ...
    PEP_PULLCASTOR_LOG(critical) << "Error with client configuration file: " << e.what();
    throw;
  }
  ...
  try {
    ...
    PEP_PULLCASTOR_LOG(critical) << "Error with OAuthToken file: " << e.what();
    PEP_PULLCASTOR_LOG(critical) << "OAuthToken is being read from " << oauthTokenFile << std::endl;
    throw;
  }
...
rxcpp::observable<size_t> EnvironmentPuller::pull() {
  ...
  if (mDry) {
    PEP_PULLCASTOR_LOG(info) << "Performing a dry run: no data will be stored in PEP";
  }
  else {
    PEP_PULLCASTOR_LOG(info) << "Performing an import run: PEP will be updated with data retrieved from Castor";
  }
  ...
  return StudyPuller::CreateChildrenFor(self) // Create study pullers
    ...
    .tap( // Perform housekeeping
      ...
      [self, read, written, startTime]() {
        ...
        PEP_PULLCASTOR_LOG(info) << "Added/updated " << *written << " of " << *read << " entries";
      }
    );
}
  ...
  return this->getStoredData()
    ...
    .tap(
      ...
      [castor, storing]() {
        ...
        PEP_PULLCASTOR_LOG(info) << description << " participant " << castor->getColumnBoundParticipantId().getParticipantId() << " column " << castor->getColumn();
      }
    );
}
...
bool EnvironmentPuller::Pull(const Configuration& config, bool dry, const std::optional<std::vector<std::string>>& spColumns, const std::optional<std::vector<std::string>>& sps) {
  ...
  PEP_PULLCASTOR_LOG(info) << "Starting castor pull";
  auto instance = EnvironmentPuller::Create(io_context, config, dry, spColumns, sps);
  instance->pull()
    .subscribe(
      [](size_t count) { PEP_PULLCASTOR_LOG(debug) << "Written " << count << " entries"; },
      [io_context](std::exception_ptr ep) {
        PEP_PULLCASTOR_LOG(error) << "Exception occured while writing Castor data to PEP: " << rxcpp::rxu::what(ep);
        io_context->stop();
      },
      [io_context, result]() {
        PEP_PULLCASTOR_LOG(info) << "Done pulling Castor data";
        io_context->stop();
        *result = true;
      });
...
void EnvironmentPuller::onCastorRequest(std::shared_ptr<const HTTPRequest> request) {
  ...
  if (position == mCastorRequests.cend()) {
    ...
    position->second++;
    PEP_PULLCASTOR_LOG(debug) << "Sending Castor request no. " << position->second << " to " << uri;
  }
}
...
rxcpp::observable<std::shared_ptr<StoredData>> EnvironmentPuller::getStoredData() {
  ...
  PEP_PULLCASTOR_LOG(info) << "Retrieving stored data from PEP";
  return this->getClient().op(RxGetOne("client"))
    ...
    return StoredData::Load(client, pps, spColumns, nonSpColumns)
      ...
      PEP_PULLCASTOR_LOG(info) << "Retrieved stored data from PEP";
