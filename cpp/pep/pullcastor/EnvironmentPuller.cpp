...
rxcpp::observable<std::shared_ptr<Client>> EnsureEnrolled(std::shared_ptr<Client> client, const std::string& token) {
  if (client->getEnrolled()) {
    return rxcpp::observable<>::just(client);
  }
  return client->enrollUser(token)
    .map([client](EnrollmentResult) {return client; });
}
...
EnvironmentPuller::EnvironmentPuller(std::shared_ptr<boost::asio::io_service> ioService, const std::filesystem::path& configFile, bool dry, const std::optional<std::vector<std::string>>& spColumns, const std::optional<std::vector<std::string>>& sps)
  : mDry(dry), mSps(sps) {
  ...
  try {
    ...
    PULLCASTOR_LOG(critical) << "Config is being read from " << configFile << std::endl;
    throw;
  }

  try {
    auto config = Configuration::FromFile(clientConfigFile);

    Client::Builder clientBuilder;

    clientBuilder.setCaCertFilepath(config.get<std::filesystem::path>("CACertificateFile"));
    ...
    clientBuilder.setKeyServerEndPoint(config.get<EndPoint>("KeyServer"));
    clientBuilder.setTranscryptorEndPoint(config.get<EndPoint>("Transcryptor"));

    clientBuilder.setIOservice(ioService);
  ...
  mCastor = CastorConnection::Create(castorAPIKeyFile, ioService);
...
rxcpp::observable<size_t> EnvironmentPuller::pull() {
  ...
  mCastor->notifyOnRequest([weak = WeakFrom(*this)](std::shared_ptr<const HTTPRequest> request) {
    auto self = weak.lock();
    if (self != nullptr) {
...
bool EnvironmentPuller::Pull(const std::filesystem::path& configFile, bool dry, const std::optional<std::vector<std::string>>& spColumns, const std::optional<std::vector<std::string>>& sps) {
  auto result = std::make_shared<bool>(false);
  auto ioService = std::make_shared<boost::asio::io_service>();

  PULLCASTOR_LOG(info) << "Starting castor pull";
  auto instance = EnvironmentPuller::Create(ioService, configFile, dry, spColumns, sps);
  instance->pull()
    .subscribe(
      [](size_t count) { PULLCASTOR_LOG(debug) << "Written " << count << " entries"; },
      [ioService](std::exception_ptr ep) {
