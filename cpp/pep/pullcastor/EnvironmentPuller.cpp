...
EnvironmentPuller::EnvironmentPuller(std::shared_ptr<boost::asio::io_service> ioService, const Configuration& config, bool dry, const std::optional<std::vector<std::string>>& spColumns, const std::optional<std::vector<std::string>>& sps)
  : mDry(dry), mSps(sps) {
...
bool EnvironmentPuller::Pull(const Configuration& config, bool dry, const std::optional<std::vector<std::string>>& spColumns, const std::optional<std::vector<std::string>>& sps) {
  ...
  auto instance = EnvironmentPuller::Create(ioService, config, dry, spColumns, sps);
  instance->pull()
    .subscribe(
