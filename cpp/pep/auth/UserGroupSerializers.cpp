...
UserGroup Serializer<UserGroup>::fromProtocolBuffer(proto::UserGroup&& source) const {
  ...
  if (source.has_max_auth_validity_seconds()) {
    result.maxAuthValidity = std::chrono::seconds(source.max_auth_validity_seconds());
  }
  return result;
}
...
void Serializer<UserGroup>::moveIntoProtocolBuffer(proto::UserGroup& dest, UserGroup value) const {
  *dest.mutable_name() = std::move(value.name);
  if (value.maxAuthValidity) {
    dest.set_max_auth_validity_seconds(static_cast<uint64_t>(value.maxAuthValidity->count()));
  }
}
