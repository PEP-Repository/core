...
bool OAuthToken::verify(const std::string& secret, const std::string& requiredSubject, const std::string& requiredGroup) const {
  PEP_LOG("OAuthToken::verify", debug) << "Verifying OAuth token " << mSerialized;
  PEP_LOG("OAuthToken::verify", debug) << "EncodeBase64Url(remoteJSON): " << EncodeBase64Url(mData);
  ...
  if (!const_time::IsEqual(localHMAC, mHmac)) {
    PEP_LOG("OAuthToken::verify", info) << "MAC in token invalid";
    result = false;
  }
...
bool OAuthToken::verifySubject(const std::string& required) const {
  if (required != mSubject) {
    PEP_LOG("OAuthToken::verify", info) << "Subject in token '" << mSubject << "' does not match required subject '" << required << "'";
    return false;
  }
  return true;
}
...
bool OAuthToken::verifyGroup(const std::string& required) const {
  if (required != mGroup) {
    PEP_LOG("OAuthToken::verify", info) << "Group in token '" << mGroup << "' does not match required group '" << required << "'";
    return false;
  }
  return true;
}
...
bool OAuthToken::verifyValidityPeriod() const {
  ...
  if (mIssuedAt >= now + 1min) { // Account for clock drift.  See #677
    PEP_LOG("OAuthToken::verifyValidityPeriod", info) << "Token issued after current time";
    return false;
  }

  // Check whether token already expired
  if (mExpiresAt <= now) {
    PEP_LOG("OAuthToken::verifyValidityPeriod", info) << "Token expired";
    return false;
  }
...
OAuthToken::OAuthToken(const std::string& serialized)
  ...
  if (splitToken.size() != 2) {
    PEP_LOG("OAuthToken",info) << "Invalid token format: did not match the correct format.";
  ...
  if (splitToken[1].length() != 43) {
    PEP_LOG("OAuthToken",info) << "Invalid token format: HMAC was not of the correct length.";
    throw std::runtime_error("Invalid token format.");
  }
  ...
  try {
    ...
    PEP_LOG("OAuthToken",info) << "Error decoding token: " << e.what();
    throw std::runtime_error("Invalid token format.");
  }
  catch (const boost::property_tree::json_parser_error& e) {
    PEP_LOG("OAuthToken",info) << "Error parsing JSON: " << e.what();
    throw std::runtime_error("Invalid token format.");
  }
}
