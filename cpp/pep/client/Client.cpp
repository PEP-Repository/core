...
rxcpp::observable<EnrollmentResult> Client::enrollUser(const std::string& oauthToken) {
  ...
  LOG(LOG_TAG, debug) << "Generated CSR for CN=" << csr.getCommonName() << " and OU=" << csr.getOrganizationalUnit();
...
void Client::Builder::initialize(const Configuration& config,
  ...
  CoreClient::Builder::initialize(config, io_service, persistKeysFile);
