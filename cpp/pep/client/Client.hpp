...
public:
  ...
  //
  // Authserver administration API
  //
  rxcpp::observable<FakeVoid> asaCreateUser(std::string uid);

  rxcpp::observable<FakeVoid> asaRemoveUser(std::string uid);

  rxcpp::observable<FakeVoid> asaAddUserIdentifier(std::string existingUid, std::string newUid);

  rxcpp::observable<FakeVoid> asaRemoveUserIdentifier(std::string uid);

  rxcpp::observable<FakeVoid> asaCreateUserGroup(std::string name, UserGroupProperties properties);

  rxcpp::observable<FakeVoid> asaModifyUserGroup(std::string name, UserGroupProperties properties);

  rxcpp::observable<FakeVoid> asaRemoveUserGroup(std::string name);

  rxcpp::observable<FakeVoid> asaAddUserToGroup(std::string uid, std::string group);

  rxcpp::observable<FakeVoid> asaRemoveUserFromGroup(std::string uid, std::string group);

  rxcpp::observable<AsaQueryResponse> asaQuery(AsaQuery query);

  rxcpp::observable<std::string> asaRequestToken(std::string subject, std::string group, Timestamp expirationTime);
...
private:
  ...
  std::shared_ptr<ServerConnection> clientKeyServer;
  std::shared_ptr<ServerConnection> clientAuthserver;
  std::shared_ptr<ServerConnection> clientRegistrationServer;
  ...
  rxcpp::observable<FakeVoid> asaRequestMutation(AsaMutationRequest request);
};
