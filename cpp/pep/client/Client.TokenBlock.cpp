...
rxcpp::observable<TokenBlockingCreateResponse> Client::tokenBlockCreate(
    ...
    std::string note) {
  return clientKeyServer->requestTokenBlockingCreate(TokenBlockingCreateRequest{std::move(target), std::move(note)});
}

rxcpp::observable<TokenBlockingRemoveResponse> Client::tokenBlockRemove(int64_t id) {
  return clientKeyServer->requestTokenBlockingRemove(TokenBlockingRemoveRequest{id});
}

rxcpp::observable<TokenBlockingListResponse> Client::tokenBlockList() {
  return clientKeyServer->requestTokenBlockingList();
}
