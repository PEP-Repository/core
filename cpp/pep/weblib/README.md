...
- With the `pepWeblib` target, this get compiled to a `.wasm` file plus associated `.mjs` wrapper including Emscripten runtime. Some (incomplete) TypeScript declarations are also generated, which are later amended in `/weblib/pep-repo-client-lib/src/pep.mts`.
