...
rxcpp::observable<std::string> OAuthClient::doTokenRequest(const std::string& code) {
  ...
  auto request = std::make_shared<HTTPRequest>(HTTPRequest::POST, boost::urls::url(mTokenUrl), std::move(body),
    HTTPMessage::HeaderMap{{"Content-Type", "application/x-www-form-urlencoded"}});

  return HTTPSClient::SendRequest(request, mIoContext, mCaCertFilepath).map([](HTTPResponse response) {
