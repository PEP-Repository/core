...
void ServerConnection::onConnected(std::shared_ptr<Connection> connection) {
  ...
  if (mConnection == nullptr) {
    ...
    if (!send.empty()) {
      PEP_LOG(LOG_TAG, debug) << (mNode ? mNode->describe() + ": " : "") << "Sending " << send.size() << " previously pending requests";
    }
...
rxcpp::observable<std::string> ServerConnection::sendRequest(std::shared_ptr<std::string> message, std::optional<messaging::MessageBatches> tail) {
  ...
  PEP_LOG(LOG_TAG, debug) << (mNode ? mNode->describe() + ": " : "") << "Adding request to pending requests list while waiting for connection";
