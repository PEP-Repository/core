...
Signature Signature::create(const std::string& data, X509CertificateChain chain,
  ...
  return Signature(
    privateKey.signDigest(hasher.digest().substr(0, 32)),
    std::move(chain),
    scheme,
    timestamp,
    isLogCopy
...
void Signature::assertValid(
  ...
  if (!mCertificateChain.begin()->getPublicKey().verifyDigest(
        hasher.digest().substr(0, 32), mSignature))
    throw Error("Invalid signature: data does not match signature or chain");
}

AsymmetricKey Signature::getPublicKey() const {
  if (mCertificateChain.begin() == mCertificateChain.end())
    return AsymmetricKey();
  return mCertificateChain.begin()->getPublicKey();
}

std::string Signature::getCommonName() const {
  if (mCertificateChain.begin() == mCertificateChain.end())
    return "";
  return mCertificateChain.begin()->getCommonName();
}

std::string Signature::getOrganizationalUnit() const {
  if (mCertificateChain.begin() == mCertificateChain.end())
    return "";
  return mCertificateChain.begin()->getOrganizationalUnit();
}
