...
std::optional<std::string> SearchOIDinName(X509_NAME* name, int nid) {
  ...
  if (index == -1) {
    PEP_LOG(LogTag, error) << "Failed to obtain index for NID: " << nid << " in SearchOIDinName.";
    return std::nullopt;
  } else if (index == -2) {
...
bool X509CertificateChain::verify(const X509RootCertificates& rootCAs) const { // TODO: move code to constructor; validate correct order (leaf first)
  ...
  if (result < 0) {
    ...
    PEP_LOG(LogTag, error) << "Verification failed with error string: " << diagnostic << " in X509CertificateChain::verify.";
    PEP_LOG(LogTag, error) << "Leaf certificate public key: " << leaf().getPublicKey().toPem();
    return false;
  }
...
bool X509CertificateSigningRequest::verifySignature() const {
  ...
  if (result == 1) {
    ...
    PEP_LOG(LogTag, error) << "Failed to verify CSR signature." << errors;
    return false;
  } else {
...
X509IdentityFiles::X509IdentityFiles(std::filesystem::path privateKeyFilePath, std::filesystem::path certificateChainFilePath, const X509RootCertificates& rootCas)
  ...
  PEP_LOG(LogTag, debug) << "Added X509IdentityFiles from Configuration";
