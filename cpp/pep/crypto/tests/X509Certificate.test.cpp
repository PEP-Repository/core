...
std::string rootCACertPEM = "-----BEGIN CERTIFICATE-----\n\
...
std::string serverCAPrivateKeyPEM = "-----BEGIN PRIVATE KEY-----\n\
...
std::string serverCACertPEM = "-----BEGIN CERTIFICATE-----\n\
...
std::string authserverCertPEM = "-----BEGIN CERTIFICATE-----\n\
...
std::string accessmanagerTLSCertPEM = "-----BEGIN CERTIFICATE-----\n\
...
std::vector<unsigned char> caCertificateDerVector= {
...
std::string caCertificateDer(reinterpret_cast<char const*>(caCertificateDerVector.data()), caCertificateDerVector.size());

std::string rootCACertPEMExpired = "-----BEGIN CERTIFICATE-----\n\
MIIF8zCCA9ugAwIBAgIUUrSjvbBwXmyPrRza3ivlcpt3SrcwDQYJKoZIhvcNAQEL\n\
BQAwgYAxCzAJBgNVBAYTAk5MMRMwEQYDVQQIDApHZWxkZXJsYW5kMREwDwYDVQQH\n\
DAhOaWptZWdlbjEdMBsGA1UECgwUUmFkYm91ZCBVbml2ZXJzaXRlaXQxFDASBgNV\n\
BAsMC1BFUCBSb290IENBMRQwEgYDVQQDDAtQRVAgUm9vdCBDQTAeFw0yNDEwMzAx\n\
...
std::string serverCACertPEMWithExpiredRoot = "-----BEGIN CERTIFICATE-----\n\
...
std::string authserverCertPEMWithExpiredRoot = "-----BEGIN CERTIFICATE-----\n\
...
TEST(X509CertificateTest, CopyConstructor) {
  pep::X509Certificate cert1(serverCACertPEM);
  pep::X509Certificate cert2(cert1);
  EXPECT_EQ(cert1.toPem(), cert2.toPem());
}

TEST(X509CertificateTest, AssignmentOperator) {
  pep::X509Certificate cert1(serverCACertPEM);
  pep::X509Certificate cert2;
  cert2 = cert1;
  EXPECT_EQ(cert1.toPem(), cert2.toPem());
}

TEST(X509CertificateTest, GetPublicKey) {
  pep::X509Certificate cert(serverCACertPEM);
  pep::AsymmetricKey publicKey = cert.getPublicKey();
...
TEST(X509CertificateTest, GetCommonName) {
  ...
  pep::AsymmetricKey caPrivateKey(serverCAPrivateKeyPEM);
  pep::X509Certificate caCertificate(serverCACertPEM);
  pep::X509Certificate cert = csr.signCertificate(caCertificate, caPrivateKey, 60);
...
TEST(X509CertificateTest, HasTLSServerEKU) {
  ...
  pep::X509Certificate cert(accessmanagerTLSCertPEM);
...
TEST(X509CertificateTest, IsntServerCertificate) {
  ...
  EXPECT_FALSE(cert.isServerCertificate()) << "Certificate is incorrectly identified as a server certificate";
}
...
TEST(X509CertificateTest, IsServerCertificate) {
  ...
  EXPECT_TRUE(cert.isServerCertificate()) << "Certificate is not a server certificate.";
}

TEST(X509CertificateTest, GetValidityDuration) {
  std::string testCN = "TestCN";
  std::string testOU = "TestOU";
  pep::AsymmetricKeyPair keyPair;
  keyPair.generateKeyPair();
  ...
  uint32_t expectedDuration = 60;
  // Hardcoded duration: 60 minutes in seconds
  uint32_t expectedDuration2 = 3600;
  ...
  // Get the actual duration from the certificate
  int64_t actualDuration = cert.getValidityDuration();

  // 3 seconds tolerance
  EXPECT_NEAR(static_cast<double>(expectedDuration), static_cast<double>(actualDuration), 3.0) << "Validity duration does not match expected value";

  // Get the actual duration from the certificate
  int64_t actualDuration2 = cert2.getValidityDuration();

  // 3 seconds tolerance
  EXPECT_NEAR(static_cast<double>(expectedDuration2), static_cast<double>(actualDuration2), 3.0) << "Validity duration does not match expected value";
}
...
TEST(X509CertificateSigningRequestTest, VerifySignature) {
  ...
  EXPECT_NO_THROW(csr.verifySignature()) << "Signature verification of correct signature threw an error";
  ...
  csr = serializer.fromProtocolBuffer(std::move(proto));
  EXPECT_THROW(csr.verifySignature(), std::runtime_error) << "Signature verification of invalid signature did not throw an error";
}

TEST(X509CertificateSigningRequestTest, MultipleCNandOU) {
  std::string testCN = "TestCN,CN=TestCN2";
  std::string testOU = "TestOU,OU=TestOU2";
  pep::AsymmetricKeyPair keyPair;
  keyPair.generateKeyPair();
  pep::X509CertificateSigningRequest csr(keyPair, testCN, testOU);

  EXPECT_EQ(csr.getCommonName(), testCN) << "CN in CSR does not match \"invalid\" input";
  EXPECT_EQ(csr.getOrganizationalUnit(), testOU) << "OU in CSR does not match \"invalid\" input";
  ...
  EXPECT_EQ(cert.getCommonName(), csr.getCommonName()) << "CN in certificate does not match the one in CSR";
  EXPECT_EQ(cert.getOrganizationalUnit(), csr.getOrganizationalUnit()) << "OU in certificate does not match the one in CSR";
}
...
TEST(X509CertificateSigningRequestTest, UTF8CharsInUTFField) {
  ...
  std::string UTF8TestOU = std::string("Č̵͇̦̝͈͍͋͒̈̇̓̚͝t̷̛̯̫̱̰̹̿̽̇̊ḩ̷̛̼̩̜͌̔̊̔̑͆̀ṳ̸̳͙̻̽̊̑͛ḷ̴̟͙̹͉̠͖̲͌̅͂̑̂̌͘͝ĥ̷̟̲̹̫̜̪̲̓̀͂̑͊̀̌̃ͅų̴̹͍̯̫͒̽ ̵̦̞̆R̵̬͌̄̇͆͊̾̿̚͝͝î̵̪̟͇͚̼͔ͅs̶̨̛͈͇̮̘̪͓̓̓̔̌̓̿̕̚e̸͖̟͚̝͔̯̎͒̾̍̓̓̂̉̐͝ŝ̵̖̹̣̪͎̔̈́̒̓͗̆̚͠"); // He comes
...
// some test that adds a very long string to a field
TEST(X509CertificateSigningRequestTest, LongStringInField) {
  std::string testCN = std::string(1000, 'A'); // Very long string
  std::string testOU = "TestOU";
  ...
  // Verify that the CN field contains the very long string
  EXPECT_EQ(csr.getCommonName(), testCN) << "CN in CSR does not match long string input";
  ...
  // Verify that the CN field in the certificate contains the very long string
  EXPECT_EQ(cert.getCommonName(), testCN) << "CN in certificate does not match long string input";
}
...
TEST(X509CertificatesTest, X509CertificatesFormatting) {
  ...
  // Next line should not throw, but does in our mbedtls implementation
  EXPECT_THROW(pep::X509Certificates certificates("extra text\n" + authserverCertPEM + serverCACertPEM), std::runtime_error);
...
TEST(X509CertificatesTest, X509CertificatesDer) {
  EXPECT_NO_THROW(pep::X509Certificates certificates(caCertificateDer));
}

TEST(X509CertificatesTest, X509CertificatesDerCorrupted) {

  std::vector<unsigned char> mangledDerData = caCertificateDerVector;

  mangledDerData.resize(mangledDerData.size() - 10);

  std::string mangledDer(reinterpret_cast<const char*>(mangledDerData.data()), mangledDerData.size());

  EXPECT_THROW(pep::X509Certificates certificates(mangledDer), std::runtime_error);
}
...
TEST(X509CertificateChainTest, VerifyCertificateChainRootCAExpired) {
  // Load the root CA certificate
