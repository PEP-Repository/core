...
typedef std::optional<std::string> ProvidedValue;
typedef std::vector<ProvidedValue> ProvidedValues;
typedef std::unordered_map<std::string, ProvidedValues> LexedValues;

class Values {
private:
  typedef std::vector<std::any> Implementor;
  Implementor mImplementor;

public:
  typedef Implementor::iterator iterator;
  typedef Implementor::const_iterator const_iterator;
