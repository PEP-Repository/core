...
class Application : public commandline::Command {
 private:
  ...
  static void ReportTermination(const std::string& reason);
 ...
 protected:
  ...
  Configuration loadMainConfigFile(const std::filesystem::path& path);
 ...
 public:
  ...
  void tryLoadConfigVersion(const std::filesystem::path& directory);
  ...
  public:
    virtual inline ~UserNotificationChannel() noexcept {}
