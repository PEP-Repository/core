...
void BacktraceSignalHandler(int dwSignalNumber, siginfo_t* lpSignalInfo, void* lpArguments) {
  ...
  case SIGSEGV:
  case SIGBUS:
    fprintf (lpCrashReportFile, "Faulting address: 0x%0*" PRIxPTR "\n", static_cast<int>(sizeof(void*) * 2), reinterpret_cast<intptr_t>(lpSignalInfo->si_addr));
    break;
  }
  ...
  fprintf (lpCrashReportFile, "Time: %s, %02d %s %04d %02d:%02d:%02d UTC (Unix %lu)\n",
...
//     fprintf (lpCrashReportFile, "============== %s ==============\n", function_name);
    ...
    fprintf (lpCrashReportFile, "#%-2d 0x%0*" PRIxPTR " in %s+0x%lu\n", i, static_cast<int>(sizeof(void*) * 2), static_cast<intptr_t>(ip), symbolName, offset);
...
//     fprintf (lpCrashReportFile, "General register states:\nRAX = %llx, RBX = %llx, RCX = %llx, RDX = %llx, RSI = %llx, RDI = %llx, RBP = %llx, RSP = %llx,\nR8 = %llx, R9 = %llx, R10 = %llx, R11 = %llx, R12 = %llx, R13 = %llx, R14 = %llx, R15 = %llx\n", RAX, RBX, RCX, RDX, RSI, RDI, RBP, RSP, R8, R9, R10, R11, R12, R13, R14, R15);
  ...
  fprintf (lpCrashReportFile, "      R8 : %016llx\n", lpContext->uc_mcontext.gregs[REG_R8]);
  fprintf (lpCrashReportFile, "      R9 : %016llx\n", lpContext->uc_mcontext.gregs[REG_R9]);
  fprintf (lpCrashReportFile, "     R10 : %016llx\n", lpContext->uc_mcontext.gregs[REG_R10]);
  fprintf (lpCrashReportFile, "     R11 : %016llx\n", lpContext->uc_mcontext.gregs[REG_R11]);
  fprintf (lpCrashReportFile, "     R12 : %016llx\n", lpContext->uc_mcontext.gregs[REG_R12]);
  fprintf (lpCrashReportFile, "     R13 : %016llx\n", lpContext->uc_mcontext.gregs[REG_R13]);
  fprintf (lpCrashReportFile, "     R14 : %016llx\n", lpContext->uc_mcontext.gregs[REG_R14]);
  fprintf (lpCrashReportFile, "     R15 : %016llx\n", lpContext->uc_mcontext.gregs[REG_R15]);
  fprintf (lpCrashReportFile, "     RDI : %016llx\n", lpContext->uc_mcontext.gregs[REG_RDI]);
  fprintf (lpCrashReportFile, "     RSI : %016llx\n", lpContext->uc_mcontext.gregs[REG_RSI]);
  fprintf (lpCrashReportFile, "     RBP : %016llx\n", lpContext->uc_mcontext.gregs[REG_RBP]);
  fprintf (lpCrashReportFile, "     RBX : %016llx\n", lpContext->uc_mcontext.gregs[REG_RBX]);
  fprintf (lpCrashReportFile, "     RDX : %016llx\n", lpContext->uc_mcontext.gregs[REG_RDX]);
  fprintf (lpCrashReportFile, "     RAX : %016llx\n", lpContext->uc_mcontext.gregs[REG_RAX]);
  fprintf (lpCrashReportFile, "     RCX : %016llx\n", lpContext->uc_mcontext.gregs[REG_RCX]);
  fprintf (lpCrashReportFile, "     RSP : %016llx\n", lpContext->uc_mcontext.gregs[REG_RSP]);
  fprintf (lpCrashReportFile, "     RIP : %016llx\n", lpContext->uc_mcontext.gregs[REG_RIP]);
  fprintf (lpCrashReportFile, "     EFL : %016llx\n", lpContext->uc_mcontext.gregs[REG_EFL]);
  fprintf (lpCrashReportFile, "  CSGSFS : %016llx\n", lpContext->uc_mcontext.gregs[REG_CSGSFS]);
  fprintf (lpCrashReportFile, "     ERR : %016llx\n", lpContext->uc_mcontext.gregs[REG_ERR]);
  fprintf (lpCrashReportFile, "  TRAPNO : %016llx\n", lpContext->uc_mcontext.gregs[REG_TRAPNO]);
  fprintf (lpCrashReportFile, " OLDMASK : %016llx\n", lpContext->uc_mcontext.gregs[REG_OLDMASK]);
  fprintf (lpCrashReportFile, "     CR2 : %016llx\n", lpContext->uc_mcontext.gregs[REG_CR2]);
#elif defined (__i386__)
  fprintf (lpCrashReportFile, "      GS : %08x\n", lpContext->uc_mcontext.gregs[REG_GS]));
  fprintf (lpCrashReportFile, "      FS : %08x\n", lpContext->uc_mcontext.gregs[REG_FS]));
  fprintf (lpCrashReportFile, "      ES : %08x\n", lpContext->uc_mcontext.gregs[REG_ES]));
  fprintf (lpCrashReportFile, "      DS : %08x\n", lpContext->uc_mcontext.gregs[REG_DS]));
  fprintf (lpCrashReportFile, "     EDI : %08x\n", lpContext->uc_mcontext.gregs[REG_EDI]));
  fprintf (lpCrashReportFile, "     ESI : %08x\n", lpContext->uc_mcontext.gregs[REG_ESI]));
  fprintf (lpCrashReportFile, "     EBP : %08x\n", lpContext->uc_mcontext.gregs[REG_EBP]));
  fprintf (lpCrashReportFile, "     ESP : %08x\n", lpContext->uc_mcontext.gregs[REG_ESP]));
  fprintf (lpCrashReportFile, "     EBX : %08x\n", lpContext->uc_mcontext.gregs[REG_EBX]));
  fprintf (lpCrashReportFile, "     EDX : %08x\n", lpContext->uc_mcontext.gregs[REG_EDX]));
  fprintf (lpCrashReportFile, "     ECX : %08x\n", lpContext->uc_mcontext.gregs[REG_ECX]));
  fprintf (lpCrashReportFile, "     EAX : %08x\n", lpContext->uc_mcontext.gregs[REG_EAX]));
  fprintf (lpCrashReportFile, "  TRAPNO : %08x\n", lpContext->uc_mcontext.gregs[REG_TRAPNO]));
  fprintf (lpCrashReportFile, "     ERR : %08x\n", lpContext->uc_mcontext.gregs[REG_ERR]));
  fprintf (lpCrashReportFile, "     EIP : %08x\n", lpContext->uc_mcontext.gregs[REG_EIP]));
  fprintf (lpCrashReportFile, "      CS : %08x\n", lpContext->uc_mcontext.gregs[REG_CS]));
  fprintf (lpCrashReportFile, "     EFL : %08x\n", lpContext->uc_mcontext.gregs[REG_EFL]));
  fprintf (lpCrashReportFile, "    UESP : %08x\n", lpContext->uc_mcontext.gregs[REG_UESP]));
  fprintf (lpCrashReportFile, "      SS : %08x\n", lpContext->uc_mcontext.gregs[REG_SS]));
#elif defined (__arm__)
  fprintf (lpCrashReportFile, "      R0 : %08lx\n", lpContext->uc_mcontext.arm_r0));
  fprintf (lpCrashReportFile, "      R1 : %08lx\n", lpContext->uc_mcontext.arm_r1));
  fprintf (lpCrashReportFile, "      R2 : %08lx\n", lpContext->uc_mcontext.arm_r2));
  fprintf (lpCrashReportFile, "      R3 : %08lx\n", lpContext->uc_mcontext.arm_r3));
  fprintf (lpCrashReportFile, "      R4 : %08lx\n", lpContext->uc_mcontext.arm_r4));
  fprintf (lpCrashReportFile, "      R5 : %08lx\n", lpContext->uc_mcontext.arm_r5));
  fprintf (lpCrashReportFile, "      R6 : %08lx\n", lpContext->uc_mcontext.arm_r6));
  fprintf (lpCrashReportFile, "      R7 : %08lx\n", lpContext->uc_mcontext.arm_r7));
  fprintf (lpCrashReportFile, "      R8 : %08lx\n", lpContext->uc_mcontext.arm_r8));
  fprintf (lpCrashReportFile, "      R9 : %08lx\n", lpContext->uc_mcontext.arm_r9));
  fprintf (lpCrashReportFile, "     R10 : %08lx\n", lpContext->uc_mcontext.arm_r10));
  fprintf (lpCrashReportFile, "      FP : %08lx\n", lpContext->uc_mcontext.arm_fp));
  fprintf (lpCrashReportFile, "      IP : %08lx\n", lpContext->uc_mcontext.arm_ip));
  fprintf (lpCrashReportFile, "      SP : %08lx\n", lpContext->uc_mcontext.arm_sp));
  fprintf (lpCrashReportFile, "      LR : %08lx\n", lpContext->uc_mcontext.arm_lr));
  fprintf (lpCrashReportFile, "      PC : %08lx\n", lpContext->uc_mcontext.arm_pc));
  fprintf (lpCrashReportFile, "    CPSR : %08lx\n", lpContext->uc_mcontext.arm_cpsr));
  fprintf (lpCrashReportFile, "  TRAPNO : %08lx\n", lpContext->uc_mcontext.trap_no));
  fprintf (lpCrashReportFile, "     ERR : %08lx\n", lpContext->uc_mcontext.error_code));
  fprintf (lpCrashReportFile, " OLDMASK : %08lx\n", lpContext->uc_mcontext.oldmask));
  fprintf (lpCrashReportFile, "   FAULT : %08lx\n", lpContext->uc_mcontext.fault_address));
#endif
...
#else /* _WIN32 */
void WindowsSignalHandler (int32_t dwSignalNumber) {
...
void InitializeUnwinder() {
  ...
  if (DIR* lpReportDirectory = opendir (szReportDirectoryPath.c_str()); lpReportDirectory == nullptr) {
    ...
    while ((lpDirEntry = readdir (lpReportDirectory)) != nullptr) {
      if (strstr (lpDirEntry->d_name, szReportFileName.c_str()) != lpDirEntry->d_name) {
        /* file name must start with same prefix */
        continue;
      } else if (lpDirEntry->d_name[szReportFileName.size()] != '_') {
        continue;
      } else if (strcmp (&lpDirEntry->d_name[strlen(lpDirEntry->d_name) - 16], "_CrashReport.txt") != 0) {
        continue;
      }
