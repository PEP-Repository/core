...
MonitorableServerBase::handleMetricsRequest(
  ...
  return BatchSingleMessage(MetricsResponse(body));
}

std::unordered_set<std::string> MonitorableServerBase::getAllowedChecksumChainRequesters() {
...
MonitorableServerBase::handleChecksumChainRequest(
  ...
  return BatchSingleMessage(std::move(resp));
}
