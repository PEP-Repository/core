...
struct httpRequestHandlerParams {
  ...
  virtual rxcpp::observable<HTTPResponse> runHandler(const HTTPRequest& request) = 0;
...
struct httpRequestHandlerParamsBasic : httpRequestHandlerParams {
  ...
  rxcpp::observable<HTTPResponse> runHandler(const HTTPRequest& request) override {
    return run_on_asio(*io_context, std::bind(func, request));
  }
};
...
static int requestHandler(struct mg_connection *conn, void *cbdata)
{
  ...
  if((params->method.empty() || params->method == requestInfo->request_method)
    ...
    try {
      ...
      HTTPRequest request(hostHeader->second, requestInfo->request_method,
      ...
      return writeResponse(conn, params->runHandler(request).as_blocking().first());
    }
