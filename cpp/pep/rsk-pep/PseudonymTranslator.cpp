...
EncryptedLocalPseudonym PseudonymTranslator::translateStep(
  ...
  return rsk_.rsk(pseudonym.mEncryption, rsk_.generateKeyFactors(recipient));
}
...
PseudonymTranslator::certifiedTranslateStep(
  ...
  auto [encryption, proof] =
      rsk_.certifiedRsk(pseudonym.mEncryption, rsk_.generateKeyFactors(recipient));
  return {encryption, proof};
}
...
void PseudonymTranslator::checkTranslationProof(
  ...
  // Don't need to check for nonzero pk, the proof is enough
  return proof.verify(
      preTranslate.mEncryption,
      postTranslate.mEncryption,
      verifiers);
}
