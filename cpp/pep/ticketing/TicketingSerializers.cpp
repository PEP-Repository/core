...
LocalPseudonyms Serializer<LocalPseudonyms>::fromProtocolBuffer(proto::LocalPseudonyms&& source) const {
  LocalPseudonyms result{
    .mAccessManager = Serialization::FromProtocolBuffer(std::move(*source.mutable_access_manager())),
    .mStorageFacility = Serialization::FromProtocolBuffer(std::move(*source.mutable_storage_facility())),
    .mPolymorphic = Serialization::FromProtocolBuffer(std::move(*source.mutable_polymorphic())),
  };

  if (source.has_access_group()) {
    result.mAccessGroup = Serialization::FromProtocolBuffer(std::move(*source.mutable_access_group()));
  }
...
void Serializer<LocalPseudonyms>::moveIntoProtocolBuffer(proto::LocalPseudonyms& dest, LocalPseudonyms value) const {
  ...
  Serialization::MoveIntoProtocolBuffer(*dest.mutable_polymorphic(), value.mPolymorphic);
  if (value.mAccessGroup)
    Serialization::MoveIntoProtocolBuffer(*dest.mutable_access_group(), *value.mAccessGroup);
}
...
Ticket2 Serializer<Ticket2>::fromProtocolBuffer(proto::Ticket2&& source) const {
  ...
  Serialization::CopyFromRepeatedProtocolBuffer(result.mPseudonyms,
...
void Serializer<Ticket2>::moveIntoProtocolBuffer(proto::Ticket2& dest, Ticket2 value) const {
  ...
  *dest.mutable_user_group() = std::move(value.mUserGroup);
  ...
  Serialization::CopyToRepeatedProtocolBuffer(*dest.mutable_pseudonyms(), std::move(value.mPseudonyms));
}
...
TicketRequest2 Serializer<TicketRequest2>::fromProtocolBuffer(proto::TicketRequest2&& source) const {
  ...
  Serialization::CopyFromRepeatedProtocolBuffer(result.mPolymorphicPseudonyms, std::move(*source.mutable_polymorphic_pseudonyms()));
  return result;
}
...
void Serializer<TicketRequest2>::moveIntoProtocolBuffer(proto::TicketRequest2& dest, TicketRequest2 value) const {
  ...
  Serialization::CopyToRepeatedProtocolBuffer(*dest.mutable_polymorphic_pseudonyms(), std::move(value.mPolymorphicPseudonyms));
}
