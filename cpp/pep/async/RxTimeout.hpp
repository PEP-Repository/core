...
typedef std::chrono::milliseconds RxAsioDuration;
 ...
 * \param io_service The I/O service on which the timer will be scheduled.
 ...
 *         work won't be removed from the io_service, a.o. needlessly eating resources and
 *         preventing the io_service::run() method from terminating.
 ...
 */
rxcpp::observable<FakeVoid> RxAsioTimer(const RxAsioDuration& duration, boost::asio::io_service& io_service, rxcpp::observe_on_one_worker observe_on);
...
public:
  /*! \brief Constructor.
    ...
    * \param io_service The I/O service that'll host the timeout timer.
    * \param observe_on The coordination to use for (on_next, on_error, on_completed) notifications.
    */
  inline RxAsioTimeout(const RxAsioDuration& duration, boost::asio::io_service& io_service, rxcpp::observe_on_one_worker observe_on)
    : mDuration(duration), mIoService(io_service), mObserveOn(observe_on) {
  }
    ...
    */
    ...
    public:
      /*! \brief Subscribes to a source observable and lets it emit items to the (outer) subscriber as long as a timeout doesn't occur.
        ...
        * \param observe_on The coordination to use for (on_next, on_error, on_completed) notifications.
        */
      void process(rxcpp::observable<TItem, SourceOperator> items, RxAsioDuration timeout_after, boost::asio::io_service& io_service, rxcpp::observe_on_one_worker observe_on) {
        ...
        items
          .subscribe_on(pep::observe_on_asio(io_service)) // Schedule the source observable on our I/O service (preventing it from e.g. blocking this thread)...
        ...
        pep::RxAsioTimer(timeout_after, io_service, observe_on)
    ...
    return CreateObservable<TItem>([items, duration = mDuration, &io_service = mIoService, observe_on = mObserveOn](rxcpp::subscriber<TItem> subscriber) {
      Implementor::Create(subscriber)->process(items, duration, io_service, observe_on);
      });
  }
