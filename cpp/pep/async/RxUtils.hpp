...
public:
  ...
  struct KeyFinder {
    typedef typename std::invoke_result<TGetKey, const TItem&>::type Key;
  };
...
public:
  ...
  rxcpp::observable<std::shared_ptr<std::unordered_map<typename KeyFinder<TItem>::Key, std::shared_ptr<std::vector<TItem>>>>> operator()(rxcpp::observable<TItem, SourceOperator> items) const {
    typedef typename KeyFinder<TItem>::Key Key;
    typedef std::vector<TItem> ItemVector;
    typedef std::unordered_map<Key, std::shared_ptr<ItemVector>> Map;
...
struct RxProvideCount {
  typedef std::function<void(size_t)> Handler;
