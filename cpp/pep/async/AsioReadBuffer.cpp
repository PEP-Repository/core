...
void AsioReadBuffer::asyncReadUntil(const std::string& delimiter, const ReadHandler& handle) {
  ...
  if (auto bufferedPart = FindLongestPrefixAtEnd(mClientBuffer, delimiter)) {
    ...
    return boost::asio::async_read_until(*mSocket, mSocketBuffer, *remainder, [self = SharedFrom(*this), remainder, delimiter, handle](const boost::system::error_code& error, std::size_t bytes_transferred) {
      if (error) {
  ...
  boost::asio::async_read_until(*mSocket, mSocketBuffer, *terminator, [self = SharedFrom(*this), terminator, delimiterLength, handle](const boost::system::error_code& error, std::size_t bytes_transferred) {
    if (error) {
      return handle(error, std::string());
    }
