...
public:
  virtual inline ~RxCache() noexcept {}
...
public:
  typedef std::function<rxcpp::observable<T>()> CreateSourceFunction;
...
template <typename CreateSource>
std::shared_ptr<RxCache<typename decltype(std::declval<CreateSource>()())::value_type>> CreateRxCache(const CreateSource& createSource) {
  typedef detail::WaitlessRxCache<typename decltype(std::declval<CreateSource>()())::value_type> TCache;
