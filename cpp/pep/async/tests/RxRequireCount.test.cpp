...
TEST(RxRequireCount, Works) {
  ...
  // causes next statement to time out: EXPECT_ANY_THROW(testutils::exhaust(io_context, rxcpp::observable<>::range(1, 5).op(RxRequireCount(6U, 8U))));
  ...
  // causes next statement to time out: EXPECT_ANY_THROW(testutils::exhaust(io_context, rxcpp::observable<>::empty<int>().op(RxRequireCount(4U))));
