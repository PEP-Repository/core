...
rxcpp::observable<FakeVoid> RxAsioTimer(const std::chrono::milliseconds& duration, boost::asio::io_service& io_service, rxcpp::observe_on_one_worker observe_on) {
   ...
   */
  auto implementor = std::make_shared<boost::asio::steady_timer>(io_service);
  ...
  return CreateObservable<FakeVoid>([implementor, expire_after](rxcpp::subscriber<FakeVoid> subscriber) {
    implementor->expires_from_now(expire_after);
    ...
    subscriber.add([implementor]() {
      implementor->cancel(); // Don't keep the io_service busy if no one is interested in the timer anymore
      });
    })
