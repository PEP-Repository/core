 ...
 * \param io_service The I/O service to associate with the timer that monitors the observable.
...
rxcpp::observable<TItem> RxEnsureProgress(boost::asio::io_service& io_service, const std::string& description, const std::chrono::milliseconds maxInactive, rxcpp::observable<TItem, SourceOperator> items) {
  // Don't expect (possibly cold) observables to start work immediately, but wait until a subscriber requests items
  return CreateObservable<TItem>([&io_service, description, maxInactive, items](rxcpp::subscriber<TItem> subscriber) {
    auto monitor = ActivityMonitor::Create(io_service, description, maxInactive);
    items.subscribe(
 ...
 */
template <typename TItem, typename SourceOperator>
rxcpp::observable<TItem> RxEnsureProgress(boost::asio::io_service& io_service, const std::string& description, rxcpp::observable<TItem, SourceOperator> items) {
  return RxEnsureProgress(io_service, description, ActivityMonitor::DEFAULT_MAX_INACTIVE, items);
}
...
RxEnsureProgress(boost::asio::io_service& io_service, const std::string& description, const std::chrono::milliseconds maxInactive, const CreateSource& createSource) {
  typedef typename decltype(std::declval<CreateSource>()(std::declval<std::shared_ptr<ActivityMonitor>>()))::value_type TItem;

  // Don't start monitoring until a subscriber requests items...
  return CreateObservable<TItem>([&io_service, description, maxInactive, createSource](rxcpp::subscriber<TItem> subscriber) {
    auto monitor = ActivityMonitor::Create(io_service, description, maxInactive);
...
template <typename CreateSource>
rxcpp::observable<typename decltype(std::declval<CreateSource>()(std::declval<std::shared_ptr<ActivityMonitor>>()))::value_type>
RxEnsureProgress(boost::asio::io_service& io_service, const std::string& description, const CreateSource& createSource) {
  return RxEnsureProgress(io_service, description, ActivityMonitor::DEFAULT_MAX_INACTIVE, createSource);
}
