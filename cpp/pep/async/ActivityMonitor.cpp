...
void ActivityMonitor::startTimer(decltype(mTimer)::duration alreadyElapsed) {
  ...
  mTimer.async_wait([weak = WeakFrom(*this)](const boost::system::error_code& ec) {
    if (ec != boost::asio::error::operation_aborted) {
      auto self = weak.lock();
      if (self == nullptr) {
        PEP_LOG(LOG_TAG, error) << "Inactivity detected for job that seems to have been completed";
      }
      else {
...
void ActivityMonitor::handleTimerExpired() {
  ...
  if (mLastActivityWhen.has_value()) {
    ...
    PEP_LOG(LOG_TAG, warning) << "Inactivity detected for job: " << mDescription
...
void ActivityMonitor::activityOccurred(const std::string& what) {
  ...
  if (mTimerRunning) {
    ...
    PEP_LOG(LOG_TAG, info) << "Activity resumed for job: " << mDescription
      << " doing: " << what;
    this->startTimer();
  }
}
