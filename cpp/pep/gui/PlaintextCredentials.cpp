...
void PlaintextCredentials::runCommandLine(const std::string& cmdLine) const {
  ...
  case 2: // Split "domain\username" into separate fields
    ...
    PEP_LOG(LogTag, pep::info) << "Splitting domain\\username specification '" << mUserName.getAddress() << "' into separate fields";
  ...
  PEP_LOG(LogTag, pep::info) << "Running command line as user '" << passedUserName << "' on domain '" << passedDomain << "'";
