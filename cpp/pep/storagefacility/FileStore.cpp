...
FileStore::FileStore(
  ...
  : mPath(metadatapath),
  mPagestore(PageStore::Create(io_service, metrics_registry, pageStoreConfig))
{
...
rxcpp::observable<std::shared_ptr<std::string>> FileStore::Entry::readPage(size_t index) {
  const auto& content = this->content();
  if (content == nullptr) {
