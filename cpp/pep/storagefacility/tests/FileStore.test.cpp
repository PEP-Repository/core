...
struct Context {
  ...
  Context() {
    ...
    this->store = FileStore::Create(
        this->metapath().string(), config, this->io_service,
  ...
  std::shared_ptr<std::vector<T>> exhaust(rxcpp::observable<T> obs) {
    return pep::testutils::exhaust<T>(*(this->io_service), obs);
  }
};
...
TEST(FileStore, Basic) {
  ...
  change->setContent(std::make_unique<EntryContent>(polymorphicKey, 1, pep::EncryptionScheme::V3, EntryContent::Metadata()));
