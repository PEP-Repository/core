...
// internal declarations copied from S3.cpp for fine-grained testing
namespace pep::s3::authorization_header {
  struct Context {
...
namespace pep::s3::request
{
  ...
  TEST(S3, AH_get_vanilla_query_unreserved) {
    HttpRequest r("example.amazonaws.com",
        "GET", "/?-._~0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz=-._~0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz", "", {
          {"Host", "example.amazonaws.com"},
          {"X-Amz-Date", "20150830T123600Z"}
        }, std::map<std::string, std::string>(), false);
