...
namespace pep {
  ...
  TEST(PageStore, basic) {
    ...
    std::shared_ptr<s3::Client::Connection> direct_conn
        = envs.CreateS3Client(io_service)->openConnection();
    ...
    EXPECT_TRUE(testutils::exhaust<std::shared_ptr<std::string>>(
          *io_service, store->get(path))->empty());

    // we put data2 under at "path" in the backup bucket s3_test_bucket2
    EXPECT_EQ(testutils::exhaust<std::string>(*io_service,
    ...
    EXPECT_EQ(testutils::exhaust<std::string>(
          *io_service, store->put(path, data))->size(), 1);
