...
namespace pep
{
  ...
  public:
    ...
    rxcpp::observable<std::shared_ptr<std::string>>
  ...
  private:
    ...
    std::vector<std::shared_ptr<s3::Client::Connection>> connections;
    ...
    rxcpp::observable<std::shared_ptr<std::string>> get(const std::string& path,
  ...
  S3PageStore::S3PageStore(
    ...
    : PageStore(),
      client(s3::Client::Create(s3params)),
      connections(),
                               ...
                               : std::nullopt)
  {
    for (auto i = 0U; i < conn_count; i++) {
      this->connections.push_back(this->client->openConnection());
  ...
  rxcpp::observable<std::shared_ptr<std::string>>
    ...
    rxcpp::observable<std::shared_ptr<std::string>> result
  ...
  rxcpp::observable<std::shared_ptr<std::string>> S3PageStore::get(
      const std::string& _path, std::string bucket)
  {
    ...
    [self,path,bucket,post_pending=std::move(post_pending)]()
      ...
      return self->connections.at(conn_idx)->getObject(path, bucket)
        .op(RxButFirst(
      ...
      std::vector<std::shared_ptr<std::string>> page_parts)
  {
    ...
    [self,path,page_parts=std::move(page_parts),post_pending=std::move(post_pending)]()
      ...
      return self->connections.at(conn_idx)->putObject(path,
      ...
      std::shared_ptr<Configuration> config)
  {
    return std::make_shared<DualPageStore>(
        S3PageStore::Create(io_service, metrics_registry, config),
        LocalPageStore::Create(io_service, config));
  }
      ...
      std::shared_ptr<Configuration> config)
  {
    ...
    if (type == "s3") {
      return S3PageStore::Create(io_service, metrics_registry, config);
    } else if (type == "local") {
      return LocalPageStore::Create(io_service, config);
    } else if (type == "dual") {
      return DualPageStore::Create(io_service, metrics_registry, config);
    }
