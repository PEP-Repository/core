...
public:
  ...
  public:
    virtual ~EntryBase() noexcept {} // Class isn't used polymorphically, but just in case
  ...
  public:
    ...
    EpochMillis getValidFrom() const { return mValidFrom; }
    EntryHeader header() const;
    rxcpp::observable<std::shared_ptr<std::string>> readPage(size_t index);
