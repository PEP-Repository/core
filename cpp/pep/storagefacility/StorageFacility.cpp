...
StorageFacility::handleDataReadRequest2(std::shared_ptr<SignedDataReadRequest2> signedRequest) {
  ...
  private:
    ...
    std::optional<rxcpp::subscriber<MessageSequence>> mSubscriber;
    ...
    StreamContext(std::vector<std::shared_ptr<FileStore::Entry>> entries, std::shared_ptr<Metrics> metrics, std::chrono::steady_clock::time_point startTime)
      : mEntries(std::move(entries)), mMetrics(std::move(metrics)), mStartTime(std::move(startTime)) {
    }
  ...
  public:
    void emitTo(rxcpp::subscriber<MessageSequence> subscriber) {
       ...
       * This keeps the the number of pages being processed at the same under (or at)
  ...
  return CreateObservable<MessageSequence>(
    [ctx](rxcpp::subscriber<MessageSequence> subscriber) {
