...
StorageFacility::handleDataEnumerationRequest2(std::shared_ptr<SignedDataEnumerationRequest2> signedRequest) {
  ...
  return mWorkerPool->batched_map<8>(std::move(responseEntries),
    ...
    .flat_map([ctx, server = SharedFrom(*this)](std::shared_ptr<std::vector<ResponseEntry>> respEntriesPtr)
      ...
      std::vector<MessageSequence> response;
...
StorageFacility::handleDataReadRequest2(std::shared_ptr<SignedDataReadRequest2> signedRequest) {
  ...
  return CreateObservable<MessageSequence>(
    [ctx, server](rxcpp::subscriber<MessageSequence> subscriber) {
      // Process all files
...
void StorageFacility::emitPage(rxcpp::subscriber<MessageSequence> subscriber, uint32_t fileIndex, uint32_t pageIndex, MessageSequence page) const {
  subscriber.on_next(page.map([metrics = mMetrics, fileIndex, pageIndex](std::shared_ptr<std::string> contents) {
    ...
    if (returnedPage->size() >= MAX_SIZE_OF_MESSAGE) {
...
MessageBatches StorageFacility::handleDataAlterationRequest(
  ...
  const GetDataAlterationResponse& getResponse) {
    ...
    return CreateObservable<MessageSequence>(
      [ctx, server, request, tail, hasher, this, getResponse](
        rxcpp::subscriber<MessageSequence>
        subscriber) {
        tail.map(
...
MessageBatches StorageFacility::handleDataStoreRequest2(
