...
class TicketIndices {
public:
  typedef decltype(IndexList::mIndices)::value_type Index;
...
StorageFacility::Parameters::Parameters(std::shared_ptr<boost::asio::io_service> io_service, const Configuration& config)
...
void StorageFacility::Parameters::check() {
  if (!encIdKey)
...
StorageFacility::handleDataEnumerationRequest2(std::shared_ptr<SignedDataEnumerationRequest2> signedRequest) {
  ...
  return mWorkerPool->batched_map<8>(std::move(responseEntries),
    observe_on_asio(*getIOservice()),
    [ctx, cache = getCache(), this](ResponseEntry re) {
      re.entry.mPolymorphicKey = cache->rerandomize(
    ...
    .flat_map([ctx, server = SharedFrom(*this)](std::shared_ptr<std::vector<ResponseEntry>> respEntriesPtr)
      ...
      std::vector<rxcpp::observable<std::shared_ptr<std::string>>> response;
...
StorageFacility::handleMetadataReadRequest2(std::shared_ptr<SignedMetadataReadRequest2> signedRequest) {
  return rxcpp::observable<>::just(CreateObservable<std::shared_ptr<std::string>>([signedRequest, server = SharedFrom(*this)](rxcpp::subscriber<std::shared_ptr<std::string>> subscriber) {
    ...
    auto ticket = request.mTicket.open(
      rootCAs,
      signedRequest->getOrganizationalUnit(),
      "read-meta"
    );
    ...
    auto sendResponse = [subscriber, &response]() {
      ...
      if (serialized->size() >= TLSMessageProtocol::MAX_SIZE_OF_MESSAGE) {
    ...
    for (size_t i = 0; i < request.mIds.size(); i++) {
      ...
      entry.mPolymorphicKey = server->getCache()->rerandomize(sfcontent->getPolymorphicKey(), &cprng);
...
StorageFacility::handleDataReadRequest2(std::shared_ptr<SignedDataReadRequest2> signedRequest) {
  ...
  auto ctx = std::make_shared<StreamContext>();
  ctx->entries.resize(request.mIds.size());
  ctx->start_time = time;
  ...
  auto cprng = std::make_shared<CPRNG>();
  auto server = SharedFrom(*this);

  return CreateObservable<rxcpp::observable<std::shared_ptr<std::string>>>(
    [ctx, server](rxcpp::subscriber<rxcpp::observable<std::shared_ptr<std::string>>> subscriber) {
      // Process all files
      for (uint32_t fileIndex = 0U; fileIndex < ctx->entries.size(); ++fileIndex) {
        auto& sfentry = ctx->entries[fileIndex];
        assert(sfentry->content() != nullptr);
        assert(sfentry->content()->payload() != nullptr);
        auto pageCount = sfentry->content()->payload()->pageCount();
        for (uint32_t pageIndex = 0U; pageIndex < pageCount; ++pageIndex) {
          server->emitPage(subscriber, fileIndex, pageIndex, sfentry->readPage(pageIndex));
        }
      }

      server->mMetrics->dataRead_request_duration.Observe(std::chrono::duration<double>(std::chrono::steady_clock::now() - ctx->start_time).count()); // in seconds
      subscriber.on_completed();
    }
  );
}

void StorageFacility::emitPage(rxcpp::subscriber<rxcpp::observable<std::shared_ptr<std::string>>> subscriber, uint32_t fileIndex, uint32_t pageIndex, rxcpp::observable<std::shared_ptr<std::string>> page) const {
  subscriber.on_next(page.map([metrics = mMetrics, fileIndex, pageIndex](std::shared_ptr<std::string> contents) {
    ...
    if (returnedPage->size() >= TLSMessageProtocol::MAX_SIZE_OF_MESSAGE) {
      throw std::runtime_error("Data payload page too large to send out");
    }
    metrics->data_retrieved_bytes.Increment(
...
rxcpp::observable<rxcpp::observable<std::shared_ptr<std::string>>> StorageFacility::handleDataAlterationRequest(
  ...
  const GetDataAlterationResponse& getResponse) {
    ...
    auto ticket = request->mTicket.open(rootCAs,signedRequest->getOrganizationalUnit());
    ...
    return CreateObservable<rxcpp::observable<std::shared_ptr<std::string>>>(
      [ctx, server, request, chunks, hasher, this, getResponse](
        rxcpp::subscriber<rxcpp::observable<std::shared_ptr<std::string>>>
        subscriber) {
        chunks.map(
...
rxcpp::observable<rxcpp::observable<std::shared_ptr<std::string>>> StorageFacility::handleDataStoreRequest2(
  ...
  auto getEntryContent = [filestore = mFileStore](const DataStoreEntry2& entry) {
    ...
    return std::make_unique<EntryContent>(entry.mPolymorphicKey, entry.mMetadata.getBlindingTimestamp().getTime(), entry.mMetadata.getEncryptionScheme(), std::move(metadata));
  };
  ...
  return this->handleDataAlterationRequest<DataStoreRequest2>(signedRequest, chunks, false, getEntryContent, getResponse);
}
...
StorageFacility::handleMetadataStoreRequest2(std::shared_ptr<SignedMetadataUpdateRequest2> lpRequest) {
  ...
  auto ticket = request->mTicket.open(rootCAs, lpRequest->getOrganizationalUnit());
  ...
  for (const auto& entry : request->mEntries) {
    ...
    auto content = std::make_unique<EntryContent>(
      ...
      entry.mMetadata.getBlindingTimestamp().getTime(),
      entry.mMetadata.getEncryptionScheme(),
...
StorageFacility::handleDataDeleteRequest2(std::shared_ptr<SignedDataDeleteRequest2> signedRequest) {
  ...
  auto chunks = rxcpp::observable<>::empty<std::shared_ptr<std::string>>();
  return this->handleDataAlterationRequest<DataDeleteRequest2>(signedRequest, chunks, true, getEntryContent, getResponse);
}
...
StorageFacility::handleDataHistoryRequest2(std::shared_ptr<SignedDataHistoryRequest2> lpRequest) {
  ...
  auto accessGroup = lpRequest->getOrganizationalUnit();
  user_group::EnsureAccess({user_group::DataAdministrator, user_group::Watchdog}, accessGroup);
...
std::shared_ptr<EGCache> StorageFacility::getCache() {
  if (mCache == nullptr) {
    mCache = EGCache::get();
  }
  return mCache;
...
void StorageFacility::statsTimer(const boost::system::error_code& e) {
  ...
  mTimer.expires_from_now(boost::posix_time::seconds(60));
...
StorageFacility::StorageFacility(std::shared_ptr<pep::StorageFacility::Parameters> parameters)
  ...
  mFileStore(FileStore::Create(
    ...
    parameters->getIOservice(),
    mRegistry)),
  mMetrics(std::make_shared<Metrics>(mRegistry)),
  mTimer(*parameters->getIOservice()),
