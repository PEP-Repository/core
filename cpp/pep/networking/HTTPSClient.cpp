...
void HTTPSClient::Connection::notifyOnRequest(const HttpRequestNotificationFunction& notify) {
  mNotifyOnRequest.push_back(notify);
}

rxcpp::observable<HTTPResponse> HTTPSClient::Connection::sendRequest(std::shared_ptr<HTTPRequest> request) {
  for (const auto& notify : mNotifyOnRequest) {
    notify(request);
  }
  return CreateObservable<HTTPResponse>([self = SharedFrom(*this), request](rxcpp::subscriber<HTTPResponse> s){
    ...
    self->ensureSend();
  }).subscribe_on(observe_on_asio(*getClient()->getIOservice()));
}
...
rxcpp::observable<HTTPResponse> HTTPSClient::SendRequest(std::shared_ptr<HTTPRequest> request, std::shared_ptr<boost::asio::io_service> ioService, const std::optional<std::filesystem::path>& caCertFilepath) {
  assert(request->getUri().getProtocol() == "https");
  auto params = std::make_shared<Parameters>();
  params->setEndPoint(EndPoint(request->getHost(), request->getUri().getPort().value_or(443)));
  params->setIOservice(ioService);
  if(caCertFilepath) {
