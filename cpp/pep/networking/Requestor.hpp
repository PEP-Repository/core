...
private:
  ...
  void schedule(decltype(mEntries)::value_type& entry, std::optional<MessageBatches> tail);
...
public:
  ...
  */
  rxcpp::observable<std::string> send(std::shared_ptr<std::string> request, std::optional<MessageBatches> tail);
  ...
  */
  void processResponse(const std::string& recipient, const StreamId& streamId, const MessageFlags& flags, std::string body);
