...
#include <pep/networking/Scheduler.hpp>
...
private:
  std::shared_ptr<pep::Scheduler> mScheduler;
  std::map<pep::MessageId, StreamOutput> mStreams;
  ...
  void handleError(const pep::MessageId& id, std::exception_ptr error) {
    ASSERT_NE(error, nullptr) << "Scheduler sent notification of a NULL error";
    ASSERT_FALSE(id.type() == pep::MessageType::REQUEST && pep::Error::IsSerializable(error)) << "Request streams shouldn't produce exceptions of type Error (or derived)";
...
public:
  explicit FakeSender(std::shared_ptr<pep::Scheduler> scheduler)
    : mScheduler(scheduler) {
    mAvailableSubscription = mScheduler->onAvailable.subscribe([this]() {this->handleAvailable(); });
    mErrorSubscription = mScheduler->onError.subscribe([this](const pep::MessageId& id, std::exception_ptr error) {this->handleError(id, error); });
  }

  size_t count() const noexcept { return std::accumulate(mStreams.cbegin(), mStreams.cend(), size_t(0U), [](size_t total, const auto& pair) {return total + pair.second.items; }); }
...
public:
  static pep::MessageSequence MakeBatch(boost::asio::io_context& ioContext, size_t& exhaustCount, size_t index) {
...
struct Stream {
  ...
  std::shared_ptr<std::vector<pep::MessageSequence>> batches = std::make_shared<std::vector<pep::MessageSequence>>();
  ...
  Stream(boost::asio::io_context& ioContext, pep::StreamId& previousStreamId, size_t batchCount)
    : id(previousStreamId = pep::StreamId::MakeNext(previousStreamId)) {
...
void TestStreams(const std::vector<size_t> sizes) {
  ...
  auto streamId = pep::StreamId::BeforeFirst();
  ...
  size_t items = std::accumulate(streams.cbegin(), streams.cend(), size_t(0U), [](size_t total, const Stream& stream) {return total + stream.itemCount(); });

  auto scheduler = pep::Scheduler::Create(ioContext);
...
pep::MessageBatches TopLevelFailureDuringItemEmission() {
  auto batches = pep::CreateObservable<pep::MessageSequence>([](rxcpp::subscriber<pep::MessageSequence> batchesSubscriber) {
...
TEST(Scheduler, HandlesTopLevelFailureDuringItemEmission) {
  ...
  auto scheduler = pep::Scheduler::Create(ioContext);
  FakeSender sender(scheduler);
  scheduler->push(streamId = pep::StreamId::MakeNext(streamId), TopLevelFailureDuringItemEmission());
