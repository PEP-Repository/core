...
MessageId Scheduler::push(const StreamId& streamId, std::shared_ptr<std::string> request, std::optional<MessageBatches> tail) {
  ...
  assert(!this->isScheduledMessageId(result));

  if (!tail) {
    // We'll only send a single message: close stream immediately
    this->emplaceOutgoing(result, MessageFlags::MakePayload(true), request);
  } else {
    ...
    this->emplaceOutgoing(result, MessageFlags::MakePayload(false), request);
...
void Scheduler::emplaceOutgoing(const MessageId& messageId, const MessageFlags& flags, std::shared_ptr<std::string> message) {
  ...
  batch.messages
    ...
    .subscribe(
      ...
      [messageId, self](std::shared_ptr<std::string> message) {
        self->emplaceOutgoing(messageId, MessageFlags::MakePayload(), message);
      },
      ...
      [messageId, self](std::exception_ptr e) {
        ...
        if (e == nullptr) {
          ...
          try {
            ...
            // The exception didn't inherit from Error: don't send back details (keep serialized = nullptr)
          }
        }

        self->onError.notify(messageId, e);

        if (serialized == nullptr) {
          serialized = MakeSharedCopy(Serialization::ToString(Error{"Internal server error"}));
        }
        self->mGenerators.erase(messageId);
        self->emplaceOutgoing(messageId, MessageFlags::MakeError(), serialized);
      },
        ...
        if (sendClose) {
          ...
          for (auto it = self->mOutgoing.rbegin(); it != self->mOutgoing.rend(); ++it) {
            if (it->properties.messageId() == messageId) {
              it->properties = MessageProperties(it->properties.messageId(), it->properties.flags() | MessageFlags::MakeClose());
              adjustedInlinePayload = true;
              break;
            }
          }
          if (!adjustedInlinePayload) {
            self->emplaceOutgoing(messageId, MessageFlags::MakeClose(), std::make_shared<std::string>(""));
          }
        } else {
