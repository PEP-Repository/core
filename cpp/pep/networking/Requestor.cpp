...
Requestor::Entry::Entry(std::shared_ptr<std::string> message,
  ...
  : message(message), resendable(resendable), subscriber(subscriber) {
}
...
rxcpp::observable<std::string> Requestor::send(std::shared_ptr<std::string> request, std::optional<MessageBatches> tail) {
  return CreateObservable<std::string>([self = SharedFrom(*this), request, tail](rxcpp::subscriber<std::string> s) {
    auto streamId = self->getNewRequestStreamId();

    auto resendable = !tail.has_value(); // currently we cannot re-generate tail messages already sent, see #1225
    auto emplacement = self->mEntries.emplace(streamId, Entry(request, resendable, s));
    assert(emplacement.second);

    self->schedule(*emplacement.first, tail);
    })
    .subscribe_on(observe_on_asio(mIoContext));
}

void Requestor::processResponse(const std::string& recipient, const StreamId& streamId, const MessageFlags& flags, std::string body) {
...
void Requestor::resend() {
  for (auto& entry : mEntries) {
    ...
    if (request.resendable) {
      ...
      this->schedule(entry, std::nullopt);
    }
  }
}

void Requestor::schedule(decltype(mEntries)::value_type& entry, std::optional<MessageBatches> tail) {
  const auto& streamId = entry.first;
  auto& request = entry.second;
  mScheduler.push(streamId, request.message, tail);
}
