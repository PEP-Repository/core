...
SocketReadBuffer::SocketReadBuffer()
  : asyncRead_(&SocketReadBuffer::asyncReadDirectly) {
}

std::string SocketReadBuffer::readSocketBuffer() {
  auto size = socketBuffer_.size();

  std::string result(size, '\0');

  std::istream source(&socketBuffer_);
...
void SocketReadBuffer::asyncReadUntil(StreamSocket& source, const char* delimiter, const DelimitedReadHandler& handle) {
  ...
  auto index = clientBuffer_.find(delimiter);
  if (index != std::string::npos) {
    ...
    auto result = std::move(clientBuffer_);
    ...
    if (result.size() == bytes) { // Returning the entire buffer contents
      ...
      clientBuffer_ = result.substr(bytes); // Copy excess bytes back into the client buffer...
      result.resize(bytes); // ... then crop the result to the requested length
      asyncRead_ = &SocketReadBuffer::asyncReadBuffered; // Next bytes must be read from our buffer
    }
   ...
   * state of affairs.
   */
  if (auto bufferedPart = FindLongestPrefixAtEnd(clientBuffer_, delimiter)) {
    assert(bufferedPart < delimiterLength); // Otherwise it would have been found by the call to clientBuffer_.find (in case 1)
    auto remainder = delimiter + bufferedPart;
    return source.asyncReadUntil(socketBuffer_, remainder, [self = SharedFrom(*this), &source, delimiter, handle](const boost::system::error_code& error, std::size_t bytes_transferred) {
      auto received = self->readSocketBuffer();
      if (error) {
        // Don't retain received data if an error occurred
        received = self->clientBuffer_ + received;
      ...
      self->clientBuffer_.append(received);
      assert(self->asyncRead_ == &SocketReadBuffer::asyncReadBuffered); // Next bytes must be read from our buffer
  ...
  return source.asyncReadUntil(socketBuffer_, delimiter, [self = SharedFrom(*this), delimiter, delimiterLength, handle](const boost::system::error_code& error, std::size_t bytes_transferred) {
    auto received = self->readSocketBuffer();
    if (error) {
      // Don't retain received data if an error occurred
      received = self->clientBuffer_ + received;
    ...
    auto requested = std::move(self->clientBuffer_) + received.substr(0, cut);

    // If additional data were received, store them in our client buffer for a next call
    self->clientBuffer_ = received.substr(cut);
    self->asyncRead_ = self->clientBuffer_.empty() ? &SocketReadBuffer::asyncReadDirectly : &SocketReadBuffer::asyncReadBuffered;
...
void SocketReadBuffer::asyncReadBuffered(StreamSocket& source, void* destination, size_t bytes, const RawReadHandler& handle) {
  ...
  auto buffered = clientBuffer_.size();
  if (buffered != 0U) {
    auto extract = std::min(buffered, bytes);
    memcpy(destination, clientBuffer_.data(), extract);
    ...
    if (buffered == extract) { // Returning the entire buffer contents
      ...
      clientBuffer_ = clientBuffer_.substr(extract); // Discard extracted bytes from the client buffer
    }
  ...
  assert(asyncRead_ == &SocketReadBuffer::asyncReadDirectly);
...
void SocketReadBuffer::asyncRead(StreamSocket& source, void* destination, size_t bytes, const RawReadHandler& handle) {
  assert(asyncRead_ != nullptr);
  (this->*asyncRead_)(source, destination, bytes, handle);
}
