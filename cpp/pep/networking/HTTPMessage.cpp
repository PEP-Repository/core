...
HTTPMessage::HTTPMessage(
  ...
  : headers_(std::move(headers)), bodyparts_(std::move(bodyparts)) {}
...
std::string HTTPMessage::getBody() const {
  ...
  for (const auto& bodypart : this->bodyparts_) {
    body += *bodypart;
  }
  return body;
}
...
std::shared_ptr<std::string> HTTPMessage::getBodypart() const {
  switch (this->bodyparts_.size()) {
  case 0:
    return std::make_shared<std::string>();
  case 1:
    return this->bodyparts_[0];
  default:
...
std::map<std::string, std::string> HTTPMessage::getBodyAsFormData() const {
  if (auto it = headers_.find("Content-Type"); it != headers_.end()) {
...
std::string HTTPResponse::toString() const {
  ...
  for(const auto& header : this->getHeaders()) {
