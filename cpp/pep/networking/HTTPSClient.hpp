...
class HTTPSClient : public TLSClient<TLSProtocol> {
 public:
  typedef TLSClient<TLSProtocol>::Parameters Parameters;
  ...
  public:
    ...
    /*!
     * \brief Registers a callback to be notified when an HTTPRequest is (about to be) sent.
     * \param notify The callback to notify.
     */
    void notifyOnRequest(const HttpRequestNotificationFunction& notify);
    ...
    std::shared_ptr<HTTPRequest> makeGet(const std::string& path, const bool& useBasePath = true) {
      return std::make_shared<HTTPRequest>(getEndPoint().hostname,
        HTTPRequest::GET, (useBasePath ? basePath : "") + path);
    };
    ...
    std::shared_ptr<HTTPRequest> makePost(const std::string& path,
      ...
      return std::make_shared<HTTPRequest>(getEndPoint().hostname,
        HTTPRequest::POST, (useBasePath ? basePath : "") + path, body);
    }
  ...
  private:
    ...
    ConnectionStatus currentConnectionStatus =
      ConnectionStatus(false, boost::system::errc::make_error_code(boost::system::errc::no_message), 0, 0);
    std::vector<HttpRequestNotificationFunction> mNotifyOnRequest;
   ...
   * \brief Send a HTTP request
   * \param request The Request to send
   * \param ioService The io_service to run the request on
   * \return Observable that, if no error occurs, emits exactly one Response
   */
  static rxcpp::observable<HTTPResponse> SendRequest(std::shared_ptr<HTTPRequest> request, std::shared_ptr<boost::asio::io_service> ioService,  const std::optional<std::filesystem::path>& caCertFilepath=std::nullopt);
};
