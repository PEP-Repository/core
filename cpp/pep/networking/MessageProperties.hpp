...
class MessageFlags {
public:
  MessageFlags(bool close, bool error, bool payload);
  std::strong_ordering operator<=>(const MessageFlags& other) const = default;

  static MessageFlags MakeEmpty() noexcept;
  static MessageFlags MakeError() noexcept;
  static MessageFlags MakePayload(bool close = false) noexcept;
  static MessageFlags MakeClose(bool payload = false) noexcept;
  ...
  MessageFlags operator|(const MessageFlags& other) const;

  EncodedMessageProperties encode() const noexcept;

  friend std::ostream& operator<<(std::ostream& out, MessageFlags flags);
