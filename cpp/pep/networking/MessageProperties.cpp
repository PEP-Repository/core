...
EncodedMessageProperties MessageFlags::encode() const noexcept {
...
MessageFlags MessageFlags::MakeEmpty() noexcept {
  return MessageFlags(false, false, false);
}

MessageFlags MessageFlags::MakeError() noexcept {
  return MessageFlags(true, true, false);
}

MessageFlags MessageFlags::MakePayload(bool close) noexcept {
  return MessageFlags(close, false, true);
}

MessageFlags MessageFlags::MakeClose(bool payload) noexcept {
  return MessageFlags(true, false, payload);
}

bool MessageFlags::areValid() const noexcept {
  if (mError) {
...
bool MessageFlags::empty() const noexcept {
  return !mClose && !mError && !mPayload;
}

MessageFlags::MessageFlags(bool close, bool error, bool payload)
...
MessageFlags MessageFlags::operator|(const MessageFlags& other) const {
  return MessageFlags(this->close() || other.close(), this->error() || other.error(), this->payload() || other.payload());
}

std::ostream& operator<<(std::ostream& out, MessageFlags flags) {
  bool first = true;
...
MessageId::MessageId(MessageType type, StreamId streamId)
  : mType(std::move(type)), mStreamId(std::move(streamId)) {
}
...
MessageProperties::MessageProperties(MessageId messageId, MessageFlags flags)
  : mMessageId(std::move(messageId)), mFlags(std::move(flags)) {
...
MessageProperties MessageProperties::DecodeFrom(EncodedMessageProperties properties) {
  ...
  MessageFlags flags(flagBits & FLAG_CLOSE, flagBits & FLAG_ERROR, flagBits & FLAG_PAYLOAD);
...
MessageProperties MessageProperties::MakeForControlMessage() noexcept {
  return MessageProperties(MessageId::MakeForControlMessage(), MessageFlags::MakeEmpty());
}
