...
public:
  ...
  public:
    Connection(std::shared_ptr<TLSClient> client) : TProtocol::Connection(client), mBackoff(*client->getIoContext()) {
...
# pragma GCC diagnostic pop
        ...
        boost::asio::ip::tcp::resolver resolver(*getClient()->getIoContext());
        auto endpoints = resolver.resolve(endPoint.hostname, std::to_string(endPoint.port));

        boost::asio::async_connect(this->getSocket(), endpoints, boost::bind(&Connection::startClientHandshake, SharedFrom(*this), boost::asio::placeholders::error));
      }
      catch (...) {
  ...
  protected:
    ...
    void onConnectFailed(const boost::system::error_code& error) override {
      ...
      if (const auto timeout = mBackoff.retry(boost::bind(&Connection::reconnect, SharedFrom(*this)))) {
        auto& endPoint = getEndPoint();
        LOG("TLS Client", warning) << "Retrying connecting to " << endPoint.hostname << ":" << endPoint.port
            << " in " << std::chrono::duration_cast<std::chrono::seconds>(*timeout);
      }
    }
  ...
  private:
    void initializeCertificateVerification() {
      ...
      if (endpoint.expectedCommonName.empty()) {
        ...
        LOG("TLS Client", debug) << "Using boost's default hostname verification"
          << " for " << endpoint.hostname << ":" << endpoint.port 
          << " instead of our custom code.";
        this->socket->set_verify_callback(
            boost::asio::ssl::host_name_verification(endpoint.hostname));
      } else {
