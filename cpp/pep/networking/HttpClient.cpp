...
void HttpClient::handleReadStatusLine(const DelimitedTransfer::Result& result) {
  ...
  mConnection->asyncReadUntil(TWO_CRLFS, [self = SharedFrom(*this)](const DelimitedTransfer::Result& result) {
    self->handleReadHeaders(result);
    });
}

void HttpClient::handleReadHeaders(const DelimitedTransfer::Result& result) {
  if (!result) {
    this->onError(result.exception());
    return;
  }

  assert(result->ends_with(TWO_CRLFS));
  std::istringstream responseStream(*result);
...
void HttpClient::handleReadBody(const SizedTransfer::Result& result) {
  if (!result) {
    this->onError(result.exception());
    return;
  }
