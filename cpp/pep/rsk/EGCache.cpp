...
public:
  // used by EGCache::get()
  static std::shared_ptr<EGCache> singleton;
  static std::mutex singletonMux;
...
// Although the following two symbols have already been declared,
// they have not yet been defined.
std::shared_ptr<EGCache> EGCacheImp::singleton;
std::mutex EGCacheImp::singletonMux;

std::shared_ptr<EGCache> EGCache::get() {
  std::lock_guard<std::mutex> g(EGCacheImp::singletonMux);
  if (!EGCacheImp::singleton)
    EGCacheImp::singleton = std::static_pointer_cast<EGCache>(
            std::make_shared<EGCacheImp>());
  return EGCacheImp::singleton;
}
