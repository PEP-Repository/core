...
private:
  ...
  MessageBatches handleTranscryptorRequest(std::shared_ptr<TranscryptorRequest> pRequest, MessageSequence entriesObservable);
  MessageBatches handleRekeyRequest(std::shared_ptr<RekeyRequest> pRequest);
  MessageBatches handleLogIssuedTicketRequest(std::shared_ptr<LogIssuedTicketRequest> request);
