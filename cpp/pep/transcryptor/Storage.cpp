...
static const std::string LOG_TAG("TranscryptorStorage");
...
struct PseudonymSetPseudonymRecord {
  ...
  PseudonymSetPseudonymRecord(const LocalPseudonym& pseudonym, int64_t set) : set(set) {
    this->pseudonym = Serialization::ToCharVector(pseudonym);
    RandomBytes(this->checksumNonce, 16);
  }
...
inline auto ts_create_db(const std::string& path) {
...
class TranscryptorStorageBackend {
public:
  TranscryptorStorageBackend(const std::string& path)
    : m(ts_create_db(path)) { }
  decltype(ts_create_db("")) m;
};
...
protected:
  Result calculate(std::shared_ptr<TranscryptorStorageBackend> storage, const Result& partial, uint64_t maxCheckpoint) const override {
    ...
    for (const auto& record : storage->m.iterate<TRecord>(
...
void TranscryptorStorage::ensureInitialized() {
  ...
  try {
    auto guard = this->mStorage->m.transaction_guard();
...
void TranscryptorStorage::ensureInitialized_unguarded(bool& migrated) {
  ...
  try {
    ...
    for(auto&& p : mStorage->m.sync_schema(true)) {
      ...
      switch(p.second) {
        ...
        case sync_schema_result::new_columns_added:
          break; // fine, no removal here, break switch
        default:
          LOG(LOG_TAG, error) << p.first << ": unknown sync_schema_result: "
             << p.second;
          throw Error("Cannot verify that no data was lost during "
              "synchronization of database schemas.");
      }
  ...
  if(this->mStorage->m.get_all<TicketRequestRecord>(limit(1)).empty() &&
      this->mStorage->m.get_all<MigrationRecord>(limit(1)).empty()) {

    LOG(LOG_TAG, warning) << "Detected empty database.";
    // Record current version:
    this->mStorage->m.insert(MigrationRecord(MigrationRecord::TargetVersion));
...
void TranscryptorStorage::migrate_from_v1_to_v2() {
  ...
  for (auto record : this->mStorage->m.iterate<TicketRequestRecord>()) {
  ...
  this->mStorage->m.insert(MigrationRecord(2));
}
...
int64_t TranscryptorStorage::getOrCreateModeSet(
  ...
  for (auto& record : mStorage->m.iterate<ModeSetRecord>(
      where(c(&ModeSetRecord::key) == key)))
    return record.seqno;

  // Apparently not --- create it
  auto set = mStorage->m.insert(ModeSetRecord(key));
  for (auto& mode : modes)
    mStorage->m.insert(ModeSetModeRecord(mode, set));
  return set;
}
...
int64_t TranscryptorStorage::getOrCreateColumnSet(
  ...
  // Check if the set exists
  for (auto& record : mStorage->m.iterate<ColumnSetRecord>(
      where(c(&ColumnSetRecord::key) == key)))
    return record.seqno;

  // Apparently not --- create it
  auto set = mStorage->m.insert(ColumnSetRecord(key));
  for (auto& col : cols)
    mStorage->m.insert(ColumnSetColumnRecord(col, set));
  return set;
}
...
int64_t TranscryptorStorage::getOrCreatePseudonymSet(const std::vector<LocalPseudonym>& ps) {
  ...
  // Check if the set exists
  for (auto& record : mStorage->m.iterate<PseudonymSetRecord>(
      where(c(&PseudonymSetRecord::key) == key)))
    return record.seqno;

  // Apparently not --- create it
  auto set = mStorage->m.insert(PseudonymSetRecord(key));
  for (auto& p : ps)
    mStorage->m.insert(PseudonymSetPseudonymRecord(p, set));
  return set;
}
...
std::optional<int64_t> TranscryptorStorage::getOrCreateCertificateChain(
  ...
  for (; K<chain.size(); K++, fingerprint_it += 32) {
    ...
    std::vector<int64_t> results = this->mStorage->m.select(
  ...
  while (K>0) {
    ...
    parentId = this->mStorage->m.insert(chainRecord);
  }
...
std::optional<uint64_t> TranscryptorStorage::getCurrentVersion() {
  std::vector<MigrationRecord> records
    = this->mStorage->m.get_all<MigrationRecord>();
...
std::string TranscryptorStorage::logTicketRequest(
  ...
  std::string accessGroup
      = ticketRequest.mLogSignature->getOrganizationalUnit();
...
void TranscryptorStorage::logIssuedTicket(
    ...
    Timestamp timestamp) {
  std::optional<TicketRequestRecord> reqRecord;
  for (auto& record : mStorage->m.iterate<TicketRequestRecord>(
