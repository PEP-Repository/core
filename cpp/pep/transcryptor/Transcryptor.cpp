...
Transcryptor::Parameters::Parameters(std::shared_ptr<boost::asio::io_service> io_service, const Configuration& config)
...
boost::optional<ElgamalPrivateKey> Transcryptor::Parameters::getPseudonymKey() const {
  return pseudonymKey;
}
...
void Transcryptor::Parameters::check() {
...
rxcpp::observable<rxcpp::observable<std::shared_ptr<std::string>>> Transcryptor::handleKeyComponentRequest(std::shared_ptr<SignedKeyComponentRequest> signedRequest) {
  // Generate response
...
rxcpp::observable<rxcpp::observable<std::shared_ptr<std::string>>> Transcryptor::handleTranscryptorRequest(std::shared_ptr<TranscryptorRequest> request, rxcpp::observable<std::shared_ptr<std::string>> entriesObservable) {
  auto start_time = std::chrono::steady_clock::now();
  ...
  rxcpp::observable<rxcpp::observable<std::shared_ptr<std::string>>> result = entriesObservable
...
Transcryptor::handleLogIssuedTicketRequest(
  ...
  auto hash = this->computePseudonymHash(ticket.mPseudonyms);
...
rxcpp::observable<rxcpp::observable<std::shared_ptr<std::string>>> Transcryptor::handleRekeyRequest(std::shared_ptr<RekeyRequest> pRequest) {
  if (pRequest->mClientCertificateChain.empty()) {
    throw Error("Cannot rekey for client without a certificate");
  }
  pRequest->mClientCertificateChain.verify(getRootCAs());
