...
MessageBatches Transcryptor::handleKeyComponentRequest(std::shared_ptr<SignedKeyComponentRequest> signedRequest) {
  // Generate response
...
MessageBatches Transcryptor::handleTranscryptorRequest(std::shared_ptr<TranscryptorRequest> request, MessageSequence entriesObservable) {
  auto start_time = std::chrono::steady_clock::now();
  ...
  MessageBatches result = entriesObservable
...
Transcryptor::handleLogIssuedTicketRequest(
  ...
  auto result = BatchSingleMessage(
...
MessageBatches Transcryptor::handleRekeyRequest(std::shared_ptr<RekeyRequest> pRequest) {
