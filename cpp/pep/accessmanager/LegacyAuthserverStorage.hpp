...
private:
  class Implementor {
  public:
    Implementor(const std::string& path)
      : m(detail::legacy_authserver_create_db(path)) { }
    decltype(detail::legacy_authserver_create_db("")) m;
  };
...
public:
  ...
  auto getUserIdRecords() const {
    return mStorage->m.iterate<UserIdRecord>(sqlite_orm::order_by(&UserIdRecord::seqno).asc());
  };

  auto getUserGroupRecords() const{
    return mStorage->m.iterate<UserGroupRecord>(sqlite_orm::order_by(&UserGroupRecord::seqno).asc());
  };

  auto getUserGroupUserRecords() const{
    return mStorage->m.iterate<LegacyUserGroupUserRecord>(sqlite_orm::order_by(&LegacyUserGroupUserRecord::seqno).asc());
  };
