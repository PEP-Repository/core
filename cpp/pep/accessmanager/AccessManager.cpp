...
AccessManager::Parameters::Parameters(std::shared_ptr<boost::asio::io_context> io_context, const Configuration& config)
  ...
  transcryptorParameters->setIoContext(getIoContext());
...
MessageBatches AccessManager::handleKeyComponentRequest(std::shared_ptr<SignedKeyComponentRequest> pRequest) {
  // Generate response
  ...
  //Return result
  return BatchSingleMessage(std::move(response));
}
...
std::unordered_set<std::string> AccessManager::getAllowedChecksumChainRequesters() {
  auto result = MonitorableServer::getAllowedChecksumChainRequesters();
  for (auto& authserver : UserGroup::Authserver) result.insert(authserver);
  return result;
}

MessageBatches AccessManager::handleGlobalConfigurationRequest(std::shared_ptr<GlobalConfigurationRequest> request) {
  return BatchSingleMessage(*globalConf);
}
...
AccessManager::handleTicketRequest2(std::shared_ptr<SignedTicketRequest2> signedRequest) {
  ...
  MessageBatches result =
    ...
    auto tail = CreateObservable<MessageSequence>([ctx](rxcpp::subscriber<MessageSequence> subscriber) {
      size_t ibatch = 0U;
  ...
  result = rxcpp::observable<>::empty<MessageSequence>()
    .tap(
      [](auto) { /*ignore */},
...
MessageBatches AccessManager::handleAmaMutationRequest(std::shared_ptr<SignedAmaMutationRequest> signedRequest) {
  ...
  return this->addParticipantsToGroupsForRequest(request)
    ...
    .concat_map([](FakeVoid) {return BatchSingleMessage(AmaMutationResponse()); });
}
...
MessageBatches AccessManager::handleUserQuery(std::shared_ptr<SignedUserQuery> signedRequest) {
  auto request = signedRequest->open(this->getRootCAs());
  auto accessGroup = signedRequest->getLeafCertificateOrganizationalUnit();

  return BatchSingleMessage(backend->performUserQuery(request, accessGroup));
}

MessageBatches AccessManager::handleUserMutationRequest(std::shared_ptr<SignedUserMutationRequest> signedRequest) {
  auto request = signedRequest->open(this->getRootCAs());
  auto accessGroup = signedRequest->getLeafCertificateOrganizationalUnit();

  backend->performUserMutationsForRequest(request, accessGroup);
  return BatchSingleMessage(UserMutationResponse());
}

MessageBatches
AccessManager::handleVerifiersRequest(std::shared_ptr<VerifiersRequest> request) {
  return BatchSingleMessage(VerifiersResponse(
...
AccessManager::handleColumnAccessRequest(
  ...
  return BatchSingleMessage(backend->handleColumnAccessRequest(request, userGroup));
}
...
AccessManager::handleParticipantGroupAccessRequest(
  ...
  return BatchSingleMessage(backend->handleParticipantGroupAccessRequest(request, userGroup));
}

MessageBatches AccessManager::handleColumnNameMappingRequest
(std::shared_ptr<SignedColumnNameMappingRequest> signedRequest) {
  auto request = signedRequest->open(this->getRootCAs());
  auto userGroup = signedRequest->getLeafCertificateOrganizationalUnit();

  return BatchSingleMessage(backend->handleColumnNameMappingRequest(request, userGroup));
}

MessageBatches AccessManager::
    handleMigrateUserDbToAccessManagerRequest(std::shared_ptr<SignedMigrateUserDbToAccessManagerRequest> signedRequest, MessageSequence chunksObservable) {
  ...
  return chunksObservable.reduce(
    ...
    [tmpUserDbMigrationPath, backend=this->backend](auto streamWithCleanup) -> MessageSequence {
...
MessageBatches AccessManager::handleFindUserRequest(
    std::shared_ptr<SignedFindUserRequest> signedRequest) {
  auto request = signedRequest->open(this->getRootCAs());
  auto userGroup = signedRequest->getLeafCertificateOrganizationalUnit();

  return BatchSingleMessage(backend->handleFindUserRequest(request, userGroup));
}

rxcpp::observable<rxcpp::observable<std::shared_ptr<std::string>>> AccessManager::handleStructureMetadataRequest(
    std::shared_ptr<SignedStructureMetadataRequest> signedRequest) {
  auto request = signedRequest->open(this->getRootCAs());
  auto userGroup = signedRequest->getLeafCertificateOrganizationalUnit();
...
rxcpp::observable<rxcpp::observable<std::shared_ptr<std::string>>> AccessManager::handleSetStructureMetadataRequest(
