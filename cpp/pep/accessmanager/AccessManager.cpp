...
AccessManager::Parameters::Parameters(std::shared_ptr<boost::asio::io_service> io_service, const Configuration& config)
  ...
  std::shared_ptr<TLSMessageClient::Parameters> transcryptorParameters = std::make_shared<TLSMessageClient::Parameters>();
  transcryptorParameters->setIOservice(getIOservice());
  transcryptorParameters->setEndPoint(transcryptorEndPoint);
  transcryptorParameters->setCaCertFilepath(getRootCACertificatesFilePath());
  setTranscryptor(CreateTLSClientConnection<TLSMessageClient>(transcryptorParameters));
...
/*!
  ...
  */
std::shared_ptr<TLSMessageProtocol::Connection> AccessManager::Parameters::getTranscryptor() const {
  return transcryptor;
}
/*!
  ...
  * \return The Parameters instance itself
  */
void AccessManager::Parameters::setTranscryptor(std::shared_ptr<TLSMessageProtocol::Connection> transcryptor) {
...
void AccessManager::Parameters::check() {
  if (!pseudonymKey)
...
rxcpp::observable<rxcpp::observable<std::shared_ptr<std::string>>> AccessManager::handleKeyComponentRequest(std::shared_ptr<SignedKeyComponentRequest> pRequest) {
  // Generate response
  ...
  //Return result
  return Just(std::make_shared<std::string>(Serialization::ToString(response)));
}
...
rxcpp::observable<rxcpp::observable<std::shared_ptr<std::string>>> AccessManager::handleGlobalConfigurationRequest(std::shared_ptr<GlobalConfigurationRequest> request) {
  return Just(std::make_shared<std::string>(Serialization::ToString(*globalConf)));
}
...
AccessManager::handleEncryptionKeyRequest(std::shared_ptr<SignedEncryptionKeyRequest> signedRequest) {
  auto start_time = std::chrono::steady_clock::now();
  auto userGroup = signedRequest->getOrganizationalUnit();
  ...
  return server->mWorkerPool->batched_map<8>(ticket.mPseudonyms,
    observe_on_asio(*server->getIOservice()),
      [server, localPseudonyms](LocalPseudonyms elp) -> LocalPseudonym {
    return elp.mAccessManager.decrypt(server->mPseudonymKey);
    }).flat_map([start_time, server, dwNumUnblind, lpResponse, request,
          signedRequest, recipient, localPseudonyms](
      ...
      EncryptedKey key;
        ...
        std::vector<EncryptedKey> keys)
                              -> rxcpp::observable<rxcpp::observable<std::shared_ptr<std::string>>> {
                                  ...
                                  auto rkIndices = std::make_shared<std::vector<uint32_t>>(
                                    request->mEntries.size());
      ...
      return server->transcryptor->sendRequest<RekeyResponse>(std::move(rkReq)).flat_map(
        [server, rkIndices, start_time, request, signedRequest, recipient,
            lpResponse, localPseudonyms](RekeyResponse transRespOnStack) {

                                        auto transResp = std::make_shared<RekeyResponse>(
                                          std::move(transRespOnStack));
                                        ...
                                        return server->mWorkerPool->batched_map<8>(is,
                                          observe_on_asio(*server->getIOservice()),
            [server, request, lpResponse, signedRequest, transResp, rkIndices,
              localPseudonyms, recipient](size_t i) {
                                            auto& entry = request->mEntries[i];
...
AccessManager::handleTicketRequest2(std::shared_ptr<SignedTicketRequest2> signedRequest) {
  ...
  auto userGroup = signedRequest->mSignature->getOrganizationalUnit();
  ...
  rxcpp::observable<rxcpp::observable<std::shared_ptr<std::string>>> result =
    ctx->server->mWorkerPool->batched_map<8>(std::move(indexes),
        observe_on_asio(*ctx->server->getIOservice()),
      [ctx](size_t i) {
    ...
    auto entryMessages = CreateObservable<rxcpp::observable<std::shared_ptr<std::string>>>([ctx](rxcpp::subscriber<rxcpp::observable<std::shared_ptr<std::string>>> subscriber) {
      size_t ibatch = 0U;
      for (size_t i = 0; i < ctx->tsReqEntries.mEntries.size(); i += TS_REQUEST_BATCH_SIZE) {
        ...
        typedef decltype(ctx->tsReqEntries.mEntries.cbegin())::difference_type index;
        ...
        subscriber.on_next(rxcpp::observable<>::just(std::make_shared<std::string>(Serialization::ToString(batch))));
      }
    ...
    return ctx->server->transcryptor->sendRequest(std::make_shared<std::string>(Serialization::ToString(ctx->tsReq)), entryMessages);
    ...
    if (resp.mEntries.size() != ctx->pps.size())
      throw std::runtime_error(
          "Transcryptor returned wrong number of entries");
            ...
            if (ctx->ticket.mUserGroup == user_group::DataAdministrator && !ctx->ticket.mPseudonyms.empty()) {
      ...
      LocalPseudonym localPseudonym = ctx->ticket.mPseudonyms[i].mAccessManager.decrypt(ctx->server->mPseudonymKey);
              if (ctx->ticket.mUserGroup != user_group::DataAdministrator) {
    ...
    return ctx->server->transcryptor->sendRequest<LogIssuedTicketResponse>(std::move(logReq));
  ...
  result = rxcpp::observable<>::empty<rxcpp::observable<std::shared_ptr<std::string>>>()
    .tap(
      [](auto) { /*ignore */},
...
rxcpp::observable<rxcpp::observable<std::shared_ptr<std::string>>> AccessManager::handleAmaMutationRequest(std::shared_ptr<SignedAmaMutationRequest> signedRequest) {
  AmaMutationRequest request = signedRequest->open(this->getRootCAs());
  std::string userGroup = signedRequest->getOrganizationalUnit();
  ...
  return rxcpp::observable<>::iterate(participantsMap)
    ...
    return self->transcryptor->sendRequest(std::make_shared<std::string>(Serialization::ToString(std::move(tsRequest))),
      ...
      .map([server = SharedFrom(*self), participantGroup = participantGroup, performRemove](TranscryptorResponse resp) -> FakeVoid {
             ...
             LOG(LOG_TAG, info) << std::string(performRemove ? "Removed" : "Added") + " participant " << Logging::Escape(localPseudonym.text())
               << std::string(performRemove ? "From" : "To") + " group " << Logging::Escape(participantGroup);
             return FakeVoid();
           });
                });
}
...
rxcpp::observable<rxcpp::observable<std::shared_ptr<std::string>>>
AccessManager::handleVerifiersRequest(std::shared_ptr<VerifiersRequest> request) {
  return Just(VerifiersResponse(
...
AccessManager::handleColumnAccessRequest(
  ...
  return Just(backend->handleColumnAccessRequest(request, userGroup));
}
...
AccessManager::handleParticipantGroupAccessRequest(
  ...
  return Just(backend->handleParticipantGroupAccessRequest(request, userGroup));
}

rxcpp::observable<rxcpp::observable<std::shared_ptr<std::string>>> AccessManager::handleColumnNameMappingRequest
(std::shared_ptr<SignedColumnNameMappingRequest> signedRequest) {
  auto request = signedRequest->open(this->getRootCAs());
  auto userGroup = signedRequest->getOrganizationalUnit();

  return Just(backend->handleColumnNameMappingRequest(request, userGroup));
}
