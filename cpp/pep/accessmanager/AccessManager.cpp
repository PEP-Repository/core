...
MessageBatches AccessManager::handleStructureMetadataRequest(std::shared_ptr<SignedStructureMetadataRequest> signedRequest) {
  auto request = signedRequest->open(this->getRootCAs());
  auto userGroup = signedRequest->getLeafCertificateOrganizationalUnit();
...
MessageBatches AccessManager::handleSetStructureMetadataRequest(
