 ...
 */
  ...
  */
  ...
  std::optional<std::chrono::seconds> getMaxAuthValidity(const std::string& group, Timestamp at = Timestamp::now()) const;
...
public:
  ...
  */
  ...
  std::optional<int64_t> findInternalUserId(std::string_view identifier, Timestamp at = Timestamp::now()) const;
  /// Try to find the internalId for the user that has the given identifier. Throws if not found.
  int64_t getInternalUserId(std::string_view identifier, Timestamp at = Timestamp::now()) const;
  std::optional<int64_t> findInternalUserId(const std::vector<std::string>& identifiers, Timestamp at = Timestamp::now()) const;
  std::unordered_set<std::string> getAllIdentifiersForUser(int64_t internalUserId, Timestamp at = Timestamp::now()) const;

  /* Finding userGroupIds */
  /// Try to find the internalId for the userGroup with the given name. Returns nullopt if not found.
  std::optional<int64_t> findUserGroupId(std::string_view name, Timestamp at = Timestamp::now()) const;
  int64_t getUserGroupId(std::string_view name, Timestamp at = Timestamp::now()) const;

  /* Check user group membership */
  std::vector<UserGroup> getUserGroupsForUser(int64_t internalUserId, Timestamp at = Timestamp::now()) const;
