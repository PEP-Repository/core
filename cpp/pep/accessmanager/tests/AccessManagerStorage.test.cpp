...
TEST_F(AccessManagerStorageTest, getColumnGroupColumns_deleted_column) {
  Timestamp timestampBefore = Timestamp::now();
  ...
  Timestamp timestampAfter = Timestamp::now();
...
TEST_F(AccessManagerStorageTest, executeQuery_unfiltered_groups) {
  ...
  auto response = storage->executeUserQuery({Timestamp::now(), "", ""});
...
TEST_F(AccessManagerStorageTest, executeQuery_filtered_group) {
  ...
  auto response = storage->executeUserQuery({Timestamp::now(), "Group1", ""});
  PrepareSortedMine(response);
...
TEST_F(AccessManagerStorageTest, executeQuery_filtered_user) {
  ...
  auto response = storage->executeUserQuery({Timestamp::now(), "", "User1"});
  PrepareSortedMine(response);
...
TEST_F(AccessManagerStorageTest, executeQuery_filtered_user_alt) {
  ...
  auto response = storage->executeUserQuery({Timestamp::now(), "", "-alt"});
  PrepareSortedMine(response);
  EXPECT_EQ(response.mUsers, (std::vector<QRUser>{
      {{user1, user1Alt}, {group1}},
    })) << "should return filtered users with all alt IDs with group memberships";
...
TEST_F(AccessManagerStorageTest, executeQuery_filtered_user_and_group) {
  ...
  auto response = storage->executeUserQuery({Timestamp::now(), "GroupA", "UserA"});
  PrepareSortedMine(response);
  EXPECT_EQ(response.mUsers, (std::vector<QRUser>{
      {{userA1}, {groupA1}},
...
TEST_F(AccessManagerStorageTest, setGetMetadataBasicColumn) {
  ...
  {
    {
      const auto metaMap = MetadataToMap(storage->getStructureMetadata(Timestamp::now(), StructureMetadataType::Column));
    ...
    {
      const auto metaMap = MetadataToMap(storage->getStructureMetadata(Timestamp::now(), StructureMetadataType::Column, {.subjects = {columnName}}));
    ...
    {
      const auto metaMap = MetadataToMap(storage->getStructureMetadata(Timestamp::now(), StructureMetadataType::Column, {.subjects = {columnName}, .keys = {key2}}));
      MetadataMap expected{{columnName, {{key2, value2}}}};
      EXPECT_EQ(metaMap, expected) << "getStructureMetadata should filter by a single key";
    }
    {
      const auto metaMap = MetadataToMap(storage->getStructureMetadata(Timestamp::now(), StructureMetadataType::Column, {.subjects = {columnName}, .keys = {key1, keyB1}}));
    ...
    {
      const auto metaMap = MetadataToMap(storage->getStructureMetadata(Timestamp::now(), StructureMetadataType::Column, {.subjects = {columnName}, .keys = {{groupA, ""}}}));
      MetadataMap expected{{columnName, {{key1, value1}, {key2, value2}}}};
      EXPECT_EQ(metaMap, expected) << "getStructureMetadata should filter by metadata group";
    }
    {
      const auto metaMap = MetadataToMap(storage->getStructureMetadata(Timestamp::now(), StructureMetadataType::Column, {.subjects = {columnName}, .keys = {key1, {groupB, ""}}}));
      MetadataMap expected{{columnName, {{key1, value1}, {keyB1, valueB1}}}};
  ...
  {
    const auto metaMap = MetadataToMap(storage->getStructureMetadata(Timestamp::now(), StructureMetadataType::Column, {.keys = {key1}}));
  ...
  {
    const auto metaMap = MetadataToMap(storage->getStructureMetadata(Timestamp::now(), StructureMetadataType::Column, {.subjects = {columnName}, .keys = {key1}}));
...
TEST_F(AccessManagerStorageTest, removeMetadataStructure) {
  ...
  for (const context& ctx : contexts) {
    ...
    {
      const auto metaMap = MetadataToMap(storage->getStructureMetadata(Timestamp::now(), ctx.structureType));
...
TEST_F(AccessManagerStorageTest, getSetRemoveMetadataSeparateSubjectType) {
  ...
  {
    const auto metaMap = MetadataToMap(storage->getStructureMetadata(Timestamp::now(), StructureMetadataType::ColumnGroup));
...
TEST_F(AccessManagerStorageTest, setGetMetadataColumnGroup) {
  ...
  {
    const auto metaMap = MetadataToMap(storage->getStructureMetadata(Timestamp::now(), subjectType));
...
TEST_F(AccessManagerStorageTest, getMetadataHistoric) {
  ...
  const Timestamp preRemove = Timestamp::now();
  WaitForNewTimestamp();
...
TEST_F(AccessManagerStorageTest, getMetadataKeys) {
  ...
  {
    const auto metaKeys = storage->getStructureMetadataKeys(Timestamp::now(), StructureMetadataType::Column, subject);
    ASSERT_EQ(metaKeys, std::vector{key}) << "getStructureMetadataKeys should return just-added keys";
  }
  {
    const auto metaKeys = storage->getStructureMetadataKeys(Timestamp::now(), StructureMetadataType::Column, subject2);
