...
private:
  MessageBatches handleKeyComponentRequest(std::shared_ptr<SignedKeyComponentRequest> pRequest);
  MessageBatches handleTicketRequest2(std::shared_ptr<SignedTicketRequest2> pClientRequest);
  MessageBatches handleEncryptionKeyRequest(std::shared_ptr<SignedEncryptionKeyRequest> pClientRequest);
  MessageBatches handleAmaMutationRequest(std::shared_ptr<SignedAmaMutationRequest> pRequest);
  MessageBatches handleAmaQuery(std::shared_ptr<SignedAmaQuery> request);
  MessageBatches handleUserQuery(std::shared_ptr<SignedUserQuery> signedRequest);
  MessageBatches handleUserMutationRequest(std::shared_ptr<SignedUserMutationRequest> signedRequest);
  MessageBatches handleGlobalConfigurationRequest(std::shared_ptr<GlobalConfigurationRequest> request);
  MessageBatches handleVerifiersRequest(std::shared_ptr<VerifiersRequest> request);
  MessageBatches handleColumnAccessRequest(std::shared_ptr<SignedColumnAccessRequest> request);
  MessageBatches handleParticipantGroupAccessRequest(std::shared_ptr<SignedParticipantGroupAccessRequest> request);
  MessageBatches handleColumnNameMappingRequest(std::shared_ptr<SignedColumnNameMappingRequest> signedRequest);
  MessageBatches handleMigrateUserDbToAccessManagerRequest(std::shared_ptr<SignedMigrateUserDbToAccessManagerRequest> signedRequest, MessageSequence chunksObservable);
  MessageBatches handleFindUserRequest(std::shared_ptr<SignedFindUserRequest> signedRequest);


  rxcpp::observable<rxcpp::observable<std::shared_ptr<std::string>>> handleStructureMetadataRequest(
    std::shared_ptr<SignedStructureMetadataRequest> request);
  rxcpp::observable<rxcpp::observable<std::shared_ptr<std::string>>> handleSetStructureMetadataRequest(
    std::shared_ptr<SignedSetStructureMetadataRequest> request,
...
public:
   ...
   */
  static std::vector<AmaQueryResponse> ExtractPartialColumnGroupQueryResponse(const std::vector<AmaQRColumnGroup>& columnGroups, const size_t maxSize = MAX_SIZE_OF_MESSAGE); // TODO: move out of AM's (public even!) interface
