...
MigrateUserDbToAccessManagerResponse AccessManager::Backend::Storage::migrateUserDb(
  ...
  for(auto userGroup : authserverStorage->getUserGroupRecords()) {
    ...
    mImplementor->m.insert(std::move(userGroup));
  }
...
bool AccessManager::Backend::Storage::userInGroup(int64_t internalUserId,
  ...
  return CurrentRecordExists<UserGroupRecord>(mImplementor->m,
    c(&UserGroupUserRecord::internalUserId) == internalUserId
...
int64_t AccessManager::Backend::Storage::createUserGroup(const UserGroup& userGroup) {
...
void AccessManager::Backend::Storage::modifyUserGroup(const UserGroup& userGroup) {
  if (!hasUserGroup(userGroup.mName)) {
    std::ostringstream msg;
  ...
  mImplementor->m.insert(UserGroupRecord(getUserGroupId(userGroup.mName), std::move(userGroup.mName), to_optional_uint64(userGroup.mMaxAuthValidity)));
}
