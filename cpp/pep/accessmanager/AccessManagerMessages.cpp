...
std::vector<std::string> IndexedTicket2::getColumns() const {
  std::vector<std::string> ret;
  const auto& columns = openTicketWithoutCheckingSignature()->columns_;
...
std::vector<std::string> IndexedTicket2::getModes() const {
  std::vector<std::string> ret;
  const auto& modes = openTicketWithoutCheckingSignature()->modes_;
...
std::vector<PolymorphicPseudonym> IndexedTicket2::getAccessSubjects() const {
  return GetPolymorphicPseudonyms(openTicketWithoutCheckingSignature()->accessSubjects_);
}
