...
func (t *Triple) protoPseudonymPack() []byte {
	var packed [32 * 3]byte
	copy(packed[0*32:1*32], t.b.Bytes())
	copy(packed[1*32:2*32], t.c.Bytes())
	copy(packed[2*32:3*32], t.y.Bytes())
	return packed[:]
}

func (t *Triple) protoPolymorphicPseudonym() *pep_proto.PolymorphicPseudonym {
	return &pep_proto.PolymorphicPseudonym{
		Packed: t.protoPseudonymPack(),
	}
}

func (t *Triple) protoEncryptedLocalPseudonym() *pep_proto.EncryptedLocalPseudonym {
	return &pep_proto.EncryptedLocalPseudonym{
		Packed: t.protoPseudonymPack(),
	}
}
...
func (t *Triple) setProtoPseudonymPack(packed []byte) {
	t.b.SetBytes((*[32]byte)(packed[0*32 : 1*32]))
	t.c.SetBytes((*[32]byte)(packed[1*32 : 2*32]))
	t.y.SetBytes((*[32]byte)(packed[2*32 : 3*32]))
}

func (t *Triple) setProtoPolymorphicPseudonym(v *pep_proto.PolymorphicPseudonym) {
	t.setProtoPseudonymPack(v.Packed)
}

func (t *Triple) setProtoEncryptedLocalPseudonym(v *pep_proto.EncryptedLocalPseudonym) {
	t.setProtoPseudonymPack(v.Packed)
}
