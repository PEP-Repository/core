...
func (t *TicketRequest) proto() *pep_proto.TicketRequest2 {
	...
	ret.PolymorphicPseudonyms = make([]*pep_proto.PolymorphicPseudonym,
		len(t.PolymorphicPseudonyms))
	for i := 0; i < len(t.PolymorphicPseudonyms); i++ {
		ret.PolymorphicPseudonyms[i] = t.PolymorphicPseudonyms[i].protoPolymorphicPseudonym()
	}
	ret.Modes = t.Modes
...
func (p *LocalPseudonyms) setProto(v *pep_proto.LocalPseudonyms) {
	p.AccessManager.setProtoEncryptedLocalPseudonym(v.AccessManager)
	p.StorageFacility.setProtoEncryptedLocalPseudonym(v.StorageFacility)
	p.Polymorphic.setProtoPolymorphicPseudonym(v.Polymorphic)
}
