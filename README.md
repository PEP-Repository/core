...
1. Install required packages (ubuntu packages in monospace)
    ...
    - For Qt GUI:
      ...
      - `qt6-tools-dev-tools`
      - `qt6-l10n-tools`
      - `qt6-svg-dev` or `libqt6svg6-dev`
    - For Go watchdog:
      - `golang`
      - Go protobuf compiler (Linux: `golang-goprotobuf-dev`, MacOS: `brew install protoc-gen-go`)
...
4. Build dependencies and [generate CMake presets](https://docs.conan.io/2/examples/tools/cmake/cmake_toolchain/build_project_cmake_presets.html):
   ...
   ```
   - This creates a build folder like `./build/Debug` on your system. `Debug` may also be e.g. `Release`. See [`./conanfile.py`](./conanfile.py) for options that you can pass via `-o name=value`, e.g. `-o with_client=False` to disable the Qt GUI. These options can also be put in your profile under `[options]` as e.g. `pep/*:with_client=False`.
...
2. Install Conan, e.g. via `winget install conan`
