...
function print_cmd_usage { # cli overview of parameters
  echo "Usage: md2beamerpdf.sh <switches> mdSource [pdfDestination]
...
function build_pdf {
  ...
  case $PDF_TYPE in
    "default")
      pandoc -s --pdf-engine=xelatex -V mainfont="Liberation Serif" -f markdown-implicit_figures -t pdf -V colorlinks=true -V linkcolor=blue -V urlcolor=red -V toccolor=gray "$INFILE" -o "$OUTFILE"
      ;;
    "beamer")
