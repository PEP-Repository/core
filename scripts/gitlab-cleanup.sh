...
eval set -- "$(getopt \
  --options='h' \
  --longoptions='help,api-key-file:,no-dry-run,dtap-dir:,unshallow,delete-single' \
  --name "$0" -- "$@")"
...
while true; do
  case "$1" in
    ...
    --help|-h)
      echo "Usage: '$0' --api-key-file=... [--no-dry-run] [--dtap-dir=...] [--unshallow] <command, e.g. clean-all>"
      exit ;;
    --)
      shift
      break ;;
...
gitlab_dir_api() {
  local dir="$1"
  shift
  "$scriptdir/gitlab-api.sh" "$dir" "${api_key_json_file:?Pass --api-key-file}" "$@"
}
...
prepare_git_repo() {
  if [ "$(git rev-parse --is-shallow-repository)" != false ]; then
    if [ -n "$should_unshallow" ]; then
      ...
      echo "Unshallowing shallow repository $PWD"
      git remote set-branches origin '*'
      git fetch --unshallow
    else
      fail "$PWD is a shallow repository"
    fi
  else
    git fetch
  fi
...
# Get lines with short protected branch names
...
list_protected_commits() {
  ...
  commits="$(set_opts
    ...
    # Lookup commits at the tip of each protected branch
    raw_echo_trailing_newline "$protected_branch_names" | while read -r branch; do
      refname="refs/remotes/origin/$branch"
      objectname="$(git rev-parse --revs-only --verify "$refname")"

      >&2 echo "$objectname used by protected branch $refname"
      echo "$objectname"
    done

    # Lookup commits for protected tags
    raw_echo_trailing_newline "$protected_tag_names" | while read -r tag; do
      refname="refs/tags/$tag"
      objectname="$(git rev-parse --revs-only --verify "$refname")"

      >&2 echo "$objectname used by protected tag $tag"
      echo "$objectname"
    done
  )"
...
# just like tags for commits used on the tip of a branch / git tag in PEP FOSS.
      ...
      git for-each-ref | while read -r objectname objecttype refname; do
        case "$objecttype" in
          commit|tag)
            ...
            docker_build_commit="$(git rev-parse --revs-only "$objectname:$foss_docker_build_submodule")"
            if [ -n "$docker_build_commit" ]; then
              >&2 echo "docker-build $docker_build_commit needed by $objecttype $refname"
...
clean_dtap_branches() {
  ...
  local protected_branch_names
  protected_branch_names="$(set_opts && list_protected_branch_names "${dtap_dir:?Pass --dtap-dir}")"
  ...
  obsolete_branches="$(set_opts
    ...
    while read -r refname branch_name; do
      ...
      if list_contains "$protected_branch_names" "$branch_name"; then
