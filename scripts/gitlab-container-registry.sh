...
list_repositories() {
  "$SCRIPTPATH"/gitlab-api.sh "$git_dir" "$api_key_json_file" get-multipage registry/repositories | jq --compact-output ".[]"
}
...
delete_for_branch() {
  ...
  do
    ...
    if [ "$repo_branch" = "$branch" ]; then
      ...
      echo "$repository"
      error=$("$SCRIPTPATH"/gitlab-api.sh "$git_dir" "$api_key_json_file" delete "registry/repositories/$id" 2>&1)
