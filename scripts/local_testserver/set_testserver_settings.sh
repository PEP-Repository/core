...
if [ -z "$BUILD_ARG" ]; then
  if [ -f "$SCRIPTPATH/../../build/cpp/pep/cli/pepcli" ]; then
    RELBUILDPATH="$SCRIPTPATH/../../build/cpp"
    ...
    console_alert "--buildPath  argument (build dir) missing. Failed guessing one ($SCRIPTPATH/../../build) as well (couldn't find '$SCRIPTPATH/../../build/cpp/pep/cli/pepcli')."
    print_cmd_usage
    exit
  fi
else
...
if [ -n "$CONFIG_ARG" ]; then # # If CONFIG_ARG is provided argument given (manual setting of configuration path)
  ...
  if [ ! -f "$TOKENFILE" ]; then
    echo "File $TOKENFILE does not exist in pwd."
