...
# Function to add a submodule if it doesn't already exist
add_submodule_if_not_exists() {
  ...
  if [ ! -d "$submodule_path" ]; then
    git submodule add -b "$branch" "$repo_path" "$submodule_path"
  else
    echo "Submodule $submodule_path already exists, ensuring it is on branch $branch."
    cd "$submodule_path"
    git fetch
    git checkout "$branch"
...
# Add the submodules if they don't already exist
add_submodule_if_not_exists "pep" "$TEMPLATE_CONFIG_BRANCH" "$(find_rel_path "$(pwd)" "$PEP_FOSS_REPO_DIR").git"
add_submodule_if_not_exists "dtap" "$TEMPLATE_CONFIG_BRANCH" "$(find_rel_path "$(pwd)" "$PEP_DTAP_REPO_DIR").git"
...
for NEW_PROJECT_FILE in "${NEW_PROJECT_FILES[@]}"; do
  ...
  for PLACEHOLDER_INDEX in "${!NEW_PROJECT_PLACEHOLDERS[@]}"; do
    ...
    sed "s|${NEW_PROJECT_PLACEHOLDERS[$PLACEHOLDER_INDEX]}|$ESCAPED_REPLACE|g" "$NEW_PROJECT_FILE" > "$NEW_PROJECT_FILE.tmp"
