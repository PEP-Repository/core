...
call "%createWixInstaller%" %BUILD_DIR% pepBinaries.wixlib config %CI_COMMIT_REF_NAME% %CI_PIPELINE_ID% || exit /B 1

echo Staging WiX installer.
mkdir "%BUILD_DIR%\wix\msi"
copy "%BUILD_DIR%\wix\pep.msi" "%BUILD_DIR%\wix\msi\" || exit /B 1

"%createInstallerMetaXml%" %CI_PIPELINE_ID% %CI_JOB_ID% "%BUILD_DIR%/wix/msi/" || exit /B 1
