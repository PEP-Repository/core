...
get_scp_command() {
  ...
  # Specify the `-O` switch to use the legacy SCP protocol instead of the SFTP protocol
  # to prevent scp from hanging if the receiving side doesn't support SFTP.
  # See https://gitlab.pep.cs.ru.nl/pep/ops/-/issues/259#note_41379
  echo "scp -O $(get_command_options P "$c_entry" "$c_file_dir" "$custom_user_parent_node") -r $local_path $(get_user_at_host "$c_entry" "$custom_user_parent_node"):$remote_path"
}
...
publish() {
  ...
  cmd=$(get_scp_command "$c_entry" "$c_file_dir" "$local_path" "$remote_path")
  run_cmd "$cmd"
  ...
  if [ -z "$pull_c_entry" ]; then
    ...
    echo "$push_c_json" | while read -r push_c_entry; do
      ...
      if [ -n "$script_src" ]; then
        ...
        run_cmd "$(get_scp_command "$push_c_entry" "$push_c_file_dir" "$script_src" "data-sync/" "sync-push")"
      fi
    ...
    run_cmd "$(get_scp_command "$pull_c_entry" "$pull_c_file_dir" build/data-sync . "sync-pull")"
  fi
}
...
constellation_json=$(cat "$constellation_file")
