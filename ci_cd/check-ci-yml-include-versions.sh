...
check_version_if_included() {
  ...
  if [ "$(get_gitdir_host "$git_root")" = "$(get_gitdir_host "$submod_dir")" ] ; then
    ...
    if [ -n "$ref" ]; then
      ...
      url="repository/files/$("$SCRIPTPATH"/../scripts/url.sh encode "$file")/raw?ref=$ref"
      >&2 echo Retrieving "$url" using Gitlab API
      "$SCRIPTPATH"/../scripts/gitlab-api.sh "$submod_dir" "$api_key_json_file" get "$url" > included.yml
