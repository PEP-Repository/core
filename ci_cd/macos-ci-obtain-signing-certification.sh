...
if [[ "${CI:-false}" == "true" ]]; then
  ...
  install_cert_if_missing "$GITLAB_CI_MACOS_CERTIFICATE_APP" app_cert.p12 pep.keychain identity "$GITLAB_CI_MACOS_CERTIFICATE_APP_NAME" -P "$GITLAB_CI_MACOS_CERTIFICATE_PWD" -T /usr/bin/codesign

  install_cert_if_missing "$GITLAB_CI_MACOS_CERTIFICATE_INSTALL" install_cert.p12 pep.keychain identity "$GITLAB_CI_MACOS_CERTIFICATE_INSTALL_NAME" -P "$GITLAB_CI_MACOS_CERTIFICATE_PWD" -T /usr/bin/productsign -T /usr/bin/pkgbuild -T /usr/bin/productbuild

  install_cert_if_missing "$GITLAB_CI_MACOS_CERTIFICATE_DEV_ID_CA" dev_id_ca.cer pep.keychain certificate "Developer ID Certification Authority" -A

  install_cert_if_missing "$GITLAB_CI_MACOS_CERTIFICATE_APPLE_ROOT_CA" apple_root_ca.cer pep.keychain certificate "Apple Root CA" -A
