...
"$PEP_MACOS_FOSS_DIR/installer/macOS/macos-add-config.sh" "$PEP_MACOS_PROJ_DIR/$STAGING_DIR/$MACOS_SYS_ARCH/macOS_artifacts" "$PEP_MACOS_PROJ_DIR/config" "$CI_COMMIT_REF_NAME" "$CI_PIPELINE_ID"
...
(
  ...
  "$PEP_MACOS_FOSS_DIR/installer/macOS/create-macos-installer.sh" "$MACOS_SYS_ARCH" "$PROJECT_CAPTION" "$CI_COMMIT_REF_NAME"
)
