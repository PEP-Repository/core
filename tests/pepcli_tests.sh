...
readonly LOCAL="$4"
readonly TESTS_TO_RUN="$5"
readonly TESTS_TO_SKIP="$6"
...
if should_run_test basic; then
  ...
  pepcli --oauth-token-group "Research Assessor" store -p test-participant -c Visit1.MRI.Func -i "$SYMLINK_TEST_DATA" &&
...
if should_run_test token-block; then
  ...
  pepcli --oauth-token-group "Data Administrator" asa token block list &&
