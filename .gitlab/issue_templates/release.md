...
1. [ ] In the `pep/core` repository, push your new `release-X.Y` branch and the updated `master` branch to `origin`, triggering CI pipelines in `pep/core` and followup pipelines in other repositories. The pipelines in the `pep/ops` repository will fail their `versioning-consistency` job, so:
1. [ ] In the `pep/ops` repository, update the `version.json` file to the appropriate values:
   1. [ ] in the `release-X.Y` branch.
   1. [ ] in the `master` branch.
1. [ ] In the `pep/ops` repository, push the updated `release-X.Y` and `master` branches to `origin`, triggering CI pipelines. Ensure that these pipelines succeed.
...
1. [ ] Push the updated `stable` branch to `origin`, triggering a new `stable` CI pipeline in `pep/ops` that will stop at its `Unleash` stage.
...
1. [ ] Go to Build -> Pipeline schedules and create a new schedule for the new `release-X.Y` branch. Schedule it every week on sunday (e.g. `0 4 * * 0`, see: <https://docs.gitlab.com/ee/topics/cron/>). Make sure to stagger the scheduled pipelines by an hour.
   1. [ ] in the `pep/core` repository. Set the `WEEKLY_BUILD` variable to `yes`.
...
1. [ ] In the `pep/core` repository, merge the new `release-X.Y` branch to the `latest-release` branch. It will start a followup pipeline in `pep/ops` pipeline that will stop at the manual job in its `Unleash` stage.
...
1. [ ] Update the release documentation by running the following manual jobs:
   - [ ] `publish-public-docs` and `publish-private-docs` in the release-X.Y pipeline in `pep/ops`.
   - [ ] `publish-public-docs` in the release-X.Y in `pep/core`.
