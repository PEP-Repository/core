...
## Update `version.json` in `master`

In the project root you find the file `version.json`. This contains the major and minor version numbers of our releases.
Make a commit on the `master` branch incrementing the version numbers as required by the new release.

- Major version: When you make incompatible API changes. When the major version gets incremented, the minor version is set to `0`.
